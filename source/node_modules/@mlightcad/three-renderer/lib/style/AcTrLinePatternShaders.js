...
var AcTrLinePatternShaders = /** @class */ (function () {
    function AcTrLinePatternShaders() {
    }
    /**
     * Creates line shader by given pattern.
     * Note: remember to call line.computeLineDistances() in order to make it work!
     */
    AcTrLinePatternShaders.createLineShaderMaterial = function (pattern, color, scale, viewportScaleUniform, cameraZoomUniform) {
        var totalLength = 0.0;
        var ltypeElementLenArr = [];
        for (var i = 0; i < pattern.length; i++) {
            var len = pattern[i].elementLength;
        ...
        for (var i = 0; i < ltypeElementLenArr.length; i++) {
            if (ltypeElementLenArr[i] === 0) {
                ltypeElementLenArr[i] = 0.5; /*totalLength * 0.01 * scale*/
                totalLength += ltypeElementLenArr[i];
            }
        }
        return this.createLineShaderMaterialFromScaledPattern(ltypeElementLenArr, totalLength, color, viewportScaleUniform, cameraZoomUniform);
    };
    /**
     ...
     */
    AcTrLinePatternShaders.createLineShaderMaterialFromScaledPattern = function (pattern, patternLength, color, viewportScaleUniform, cameraZoomUniform) {
    ...
    return AcTrLinePatternShaders;
}());
export { AcTrLinePatternShaders };
//# sourceMappingURL=AcTrLinePatternShaders.js.map