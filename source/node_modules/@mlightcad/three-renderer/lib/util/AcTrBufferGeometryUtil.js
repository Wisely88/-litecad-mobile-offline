...
var AcTrBufferGeometryUtil = /** @class */ (function () {
     ...
     */
    AcTrBufferGeometryUtil.recomputeLineDistanceForLineSegments = function (geometry, worldMatrix) {
        var positionAttribute = geometry.getAttribute('position');
        if (!positionAttribute || positionAttribute.count < 2) {
            geometry.deleteAttribute('lineDistance');
            return;
        }
        var vertexCount = positionAttribute.count - (positionAttribute.count % 2);
        if (vertexCount === 0) {
            geometry.deleteAttribute('lineDistance');
            return;
        }
        if (vertexCount < positionAttribute.count) {
            var trimmed = new Float32Array(vertexCount * positionAttribute.itemSize);
            trimmed.set(positionAttribute.array.subarray(0, trimmed.length));
            geometry.setAttribute('position', new THREE.BufferAttribute(trimmed, positionAttribute.itemSize));
            positionAttribute = geometry.getAttribute('position');
        }
        var existing = geometry.getAttribute('lineDistance');
        var preserveStarts = !!existing && existing.count >= vertexCount;
    ...
    AcTrBufferGeometryUtil.computeLineDistance = function (line) {
        ...
        if (geometry.index === null) {
            ...
            if (isLineSegments) {
                AcTrBufferGeometryUtil.recomputeLineDistanceForLineSegments(geometry, worldMatrix);
            }
            else {
                var lineDistances = [];
                lineDistances[0] = 0;
    ...
    AcTrBufferGeometryUtil.sanitizeGeometryPositions = function (geometry, indexStride) {
        ...
        if (!index) {
            if (geometry.hasAttribute('lineDistance')) {
                ...
                for (var vertex = 0; vertex + 1 < position.count; vertex += 2) {
                    if (!isFiniteVertex(vertex) || !isFiniteVertex(vertex + 1)) {
                        continue;
                    }
                    var base1 = vertex * position.itemSize;
                    var base2 = (vertex + 1) * position.itemSize;
                    for (var component = 0; component < position.itemSize; component++) {
                ...
                geometry.setAttribute('position', new THREE.Float32BufferAttribute(segmentPositions, position.itemSize));
