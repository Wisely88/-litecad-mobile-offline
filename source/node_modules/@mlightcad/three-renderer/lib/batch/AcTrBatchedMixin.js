...
import { AcTrBatchGeometryDefaultFlags, ascIdSort, copyAttributeData, isBatchGeometryActive, isBatchGeometryVisible, setBatchGeometryVisible } from './AcTrBatchedGeometryInfo';
import { applyBatchSlotDrawVisibility } from './drawVisibility';
import { AcTrBatchHighlightState, BATCH_SLOT_ID_ATTRIBUTE, bindBatchHighlightUniforms, ensureSlotIdAttribute, installBatchHighlightRenderer, writeSlotIdRange } from './highlight';
/** Whether any material in the batch was patched for highlight/compare shaders. */
function hasBatchHighlightMaterial(material) {
    var materials = Array.isArray(material) ? material : [material];
    return materials.some(function (entry) { return getMaterialRuntimeUserData(entry).batchHighlightPatched; });
}
/**
 ...
 */
export function getMappingStats(geometryInfo) {
    var count = geometryInfo.length;
    var size = 0;
    if (count > 0) {
 ...
 */
export function initializeGeometry(geometry, reference, maxVertexCount, maxIndexCount) {
    for (var attributeName in reference.attributes) {
        ...
        var dstAttribute = new THREE.BufferAttribute(dstArray, itemSize, normalized);
 ...
 */
export function validateGeometry(batchGeometry, geometry, typeName, checkIndex) {
    if (checkIndex) {
        if (Boolean(geometry.getIndex()) !== Boolean(batchGeometry.getIndex())) {
            throw new Error("".concat(typeName, ": All geometries must consistently have \"index\"."));
        }
    }
    for (var attributeName in batchGeometry.attributes) {
        ...
        if (!geometry.hasAttribute(attributeName)) {
            throw new Error("".concat(typeName, ": Added geometry missing \"").concat(attributeName, "\". All geometries must have consistent attributes."));
        }
        ...
        if (srcAttribute.itemSize !== dstAttribute.itemSize ||
            ...
            throw new Error("".concat(typeName, ": All attributes must have a consistent itemSize and normalized value."));
        }
    }
}
/**
 ...
 */
export function reserveGeometryId(availableGeometryIds, geometryInfoList, geometryCount, geometryInfo) {
    var geometryId;
    var nextGeometryCount = geometryCount;
    if (availableGeometryIds.length > 0) {
        availableGeometryIds.sort(ascIdSort);
        geometryId = availableGeometryIds.shift();
    ...
    return { geometryId: geometryId, geometryCount: nextGeometryCount };
}
/**
 ...
 */
export function resolveReservedCount(reservedCount, actualCount) {
...
export function assertReservedCapacity(params) {
    var typeName = params.typeName, maxVertexCount = params.maxVertexCount, vertexStart = params.vertexStart, reservedVertexCount = params.reservedVertexCount, maxIndexCount = params.maxIndexCount, _a = params.indexStart, indexStart = _a === void 0 ? -1 : _a, _b = params.reservedIndexCount, reservedIndexCount = _b === void 0 ? 0 : _b;
    if ((maxIndexCount != null &&
        indexStart !== -1 &&
        indexStart + reservedIndexCount > maxIndexCount) ||
        vertexStart + reservedVertexCount > maxVertexCount) {
        throw new Error("".concat(typeName, ": Reserved space request exceeds the maximum buffer size."));
    }
}
/**
...
export function growCapacityIfNeeded(params) {
    var currentMaxCount = params.currentMaxCount, nextStart = params.nextStart, requiredCount = params.requiredCount, growthFactor = params.growthFactor;
    var totalRequiredCount = nextStart + requiredCount;
 ...
 */
export function deleteGeometryById(geometryId, geometryInfoList, availableGeometryIds) {
    if (geometryId >= geometryInfoList.length ||
        !isBatchGeometryActive(geometryInfoList[geometryId].flags)) {
        return false;
    }
    geometryInfoList[geometryId].flags = 0 /* AcTrBatchGeometryFlags.None */;
 ...
 */
export function validateGeometryId(geometryId, geometryInfoList, typeName) {
    if (geometryId < 0 ||
        geometryId >= geometryInfoList.length ||
        !isBatchGeometryActive(geometryInfoList[geometryId].flags)) {
        throw new Error("".concat(typeName, ": Invalid geometryId ").concat(geometryId, ". Geometry is either out of range or has been deleted."));
    }
}
/**
 ...
 */
export function copyGeometryAttributes(batchGeometry, geometry, vertexStart, reservedVertexCount) {
    for (var attributeName in batchGeometry.attributes) {
        if (attributeName === BATCH_SLOT_ID_ATTRIBUTE) {
            continue;
        }
 ...
 */
export function copyGeometryIndices(batchGeometry, geometry, vertexStart, indexStart, reservedIndexCount) {
    ...
    for (var i = srcIndex.count, l = reservedIndexCount; i < l; i++) {
 ...
 */
export function applyGeometryAt(geometryInfo, batchGeometry, geometry, typeName, slotId) {
    ...
    if ((hasIndex &&
        ...
        throw new Error("".concat(typeName, ": Reserved space not large enough for provided geometry."));
    }
    var vertexStart = geometryInfo.vertexStart;
    var reservedVertexCount = geometryInfo.reservedVertexCount;
    geometryInfo.vertexCount = geometry.getAttribute('position').count;
    copyGeometryAttributes(batchGeometry, geometry, vertexStart, reservedVertexCount);
    if (hasIndex && srcIndex) {
        var indexedInfo = geometryInfo;
        indexedInfo.indexCount = srcIndex.count;
        copyGeometryIndices(batchGeometry, geometry, vertexStart, indexedInfo.indexStart, indexedInfo.reservedIndexCount);
    }
    ...
    if (isBatchGeometryActive(geometryInfo.flags)) {
        ...
        if (!isBatchGeometryVisible(geometryInfo.flags)) {
            applyBatchSlotDrawVisibility(batchGeometry, geometryInfo, false);
        }
    }
 ...
 */
export function computeBoundingBox(currentBoundingBox, geometryInfo) {
    var boundingBox = currentBoundingBox !== null && currentBoundingBox !== void 0 ? currentBoundingBox : new THREE.Box3();
    boundingBox.makeEmpty();
    for (var i = 0, l = geometryInfo.length; i < l; i++) {
        var geometry = geometryInfo[i];
        if (!isBatchGeometryActive(geometry.flags))
            continue;
 ...
 */
export function computeBoundingSphere(currentBoundingSphere, geometryInfo, getBoundingSphereAt) {
    var boundingSphere = currentBoundingSphere !== null && currentBoundingSphere !== void 0 ? currentBoundingSphere : new THREE.Sphere();
    boundingSphere.makeEmpty();
    var sphere = new THREE.Sphere();
    for (var i = 0, l = geometryInfo.length; i < l; i++) {
        if (!isBatchGeometryActive(geometryInfo[i].flags))
            continue;
 ...
 */
export function initializeRaycastObject(raycastObject, batchGeometry, material) {
 ...
 */
export function setRaycastObjectInfo(raycastObject, start, count, boundingBox, boundingSphere) {
 ...
 */
export function resetRaycastObjectInfo(raycastObject) {
 ...
 */
export function createAcTrBatchedMixin(Base, options) {
    /**
    ...
    return /** @class */ (function (_super) {
        __extends(AcTrBatchedMixinBase, _super);
        function AcTrBatchedMixinBase() {
         ...
         */
        AcTrBatchedMixinBase.prototype.validateGeometryId = function (geometryId) {
         ...
         * @throws {Error} When the id is invalid or the slot has been deleted.
         */
        AcTrBatchedMixinBase.prototype.getGeometryRangeAt = function (geometryId) {
         ...
         */
        AcTrBatchedMixinBase.prototype.computeBoundingBox = function () {
            this.boundingBox = computeBoundingBox(this.boundingBox, this._geometryInfo);
        };
        /**
         ...
         */
        AcTrBatchedMixinBase.prototype.unionActiveVisibleBoundingBoxInto = function (target, options) {
            var self = this;
            self.updateMatrixWorld(true);
            for (var i = 0; i < this._geometryCount; i++) {
                var info = this._geometryInfo[i];
                if (!isBatchGeometryActive(info.flags))
                    continue;
                if (!isBatchGeometryVisible(info.flags))
                    continue;
                if ((options === null || options === void 0 ? void 0 : options.excludeObjectIds) &&
                    info.objectId &&
                    options.excludeObjectIds.has(info.objectId)) {
                    continue;
                }
                if ((options === null || options === void 0 ? void 0 : options.includeObjectIds) &&
                    info.objectId &&
                    !options.includeObjectIds.has(info.objectId)) {
                    continue;
                }
                var bounds = this.getBoundingBoxAt(i, this._box);
                if (bounds) {
         ...
         * @returns This instance for chaining.
         */
        AcTrBatchedMixinBase.prototype.computeBoundingSphere = function () {
            var _this = this;
            this.boundingSphere = computeBoundingSphere(this.boundingSphere, this._geometryInfo, function (geometryId, target) {
                return _this.getBoundingSphereAt(geometryId, target);
            });
        };
        /**
         ...
         * @throws {Error} When the id is invalid or the slot has been deleted.
         */
        AcTrBatchedMixinBase.prototype.setVisibleAt = function (geometryId, value) {
            this.validateGeometryId(geometryId);
            if (isBatchGeometryVisible(this._geometryInfo[geometryId].flags) === value) {
                return this;
            }
            var info = this._geometryInfo[geometryId];
            var applied = applyBatchSlotDrawVisibility(this.geometry, info, value);
            if (applied) {
         ...
         * @throws {Error} When the id is invalid or the slot has been deleted.
         */
        AcTrBatchedMixinBase.prototype.getVisibleAt = function (geometryId) {
            this.validateGeometryId(geometryId);
         ...
         */
        AcTrBatchedMixinBase.prototype.setHighlightAt = function (geometryId, kind, enabled) {
            try {
                this.validateGeometryId(geometryId);
            }
            catch (_a) {
            ...
            var changed = this._highlightState.setHighlight(geometryId, kind, enabled);
            if (changed) {
         ...
         * @returns `true` when the slot mask changed.
         */
        AcTrBatchedMixinBase.prototype.setCompareRoleAt = function (geometryId, role) {
            try {
                this.validateGeometryId(geometryId);
            }
            catch (_a) {
            ...
            var changed = this._highlightState.setCompareRole(geometryId, role);
         ...
         * @returns This instance for chaining.
         */
        AcTrBatchedMixinBase.prototype.applyPackedCompareRoles = function (roles) {
            var _a;
            for (var slotId = 0; slotId < this._geometryInfo.length; slotId++) {
                var info = this._geometryInfo[slotId];
                if (!isBatchGeometryActive(info.flags)) {
                    continue;
                }
                var objectId = info.objectId;
                var role = objectId != null && objectId !== ''
                    ? ((_a = roles.get(String(objectId))) !== null && _a !== void 0 ? _a : null)
                    : null;
         ...
         * @returns This instance for chaining.
         */
        AcTrBatchedMixinBase.prototype.setCompareDisplayColors = function (options) {
         ...
         * @returns This instance for chaining.
         */
        AcTrBatchedMixinBase.prototype.flushHighlightMask = function () {
            this._highlightState.setAddressableSlotCount(this._geometryInfo.length);
            var wasDirty = this._highlightState.dirty;
            if (wasDirty) {
                this._highlightState.uploadMaskTexture();
            }
            var shouldSyncHighlightUniforms = !!this.material &&
         ...
         * @returns This instance for chaining.
         */
        AcTrBatchedMixinBase.prototype.clearHighlightSlot = function (geometryId) {
         ...
         * @returns This instance for chaining.
         */
        AcTrBatchedMixinBase.prototype.clearHighlightState = function () {
         ...
         * @returns This instance for chaining.
         */
        AcTrBatchedMixinBase.prototype.deleteGeometry = function (geometryId) {
            var deleted = deleteGeometryById(geometryId, this._geometryInfo, this._availableGeometryIds);
            if (!deleted) {
                return this;
            }
         ...
         */
        AcTrBatchedMixinBase.prototype.getObjectAt = function (batchId) {
            var object = options.createObject();
            this._initializeRaycastObject(object);
            var geometryInfo = this._geometryInfo[batchId];
            var _a = options.getDrawRange(this, geometryInfo), start = _a.start, count = _a.count;
         ...
         */
        AcTrBatchedMixinBase.prototype.intersectWith = function (geometryId, raycaster, intersects) {
         ...
         * @param intersects - Output array populated with intersection records.
         */
        AcTrBatchedMixinBase.prototype.raycast = function (raycaster, intersects) {
            this._initializeRaycastObject(this._raycastObject);
         ...
         * @returns This instance for chaining.
         */
        AcTrBatchedMixinBase.prototype.dispose = function () {
         ...
         */
        AcTrBatchedMixinBase.prototype._initializeRaycastObject = function (raycastObject) {
         ...
         */
        AcTrBatchedMixinBase.prototype._setRaycastObjectInfo = function (raycastObject, index, start, count) {
            var self = this;
         ...
         */
        AcTrBatchedMixinBase.prototype._resetRaycastObjectInfo = function (raycastObject) {
         ...
         * @param intersects - Output array populated with intersection records.
         */
        AcTrBatchedMixinBase.prototype._intersectWith = function (geometryId, raycaster, intersects) {
            ...
            if (geometryInfo.bboxIntersectionCheck) {
                ...
                if (raycaster.ray.intersectBox(this._box, this._vector)) {
                    var distance = raycaster.ray.origin.distanceTo(this._vector);
                    intersects.push({
                        distance: distance,
                ...
                var _a = options.getDrawRange(this, geometryInfo), start = _a.start, count = _a.count;
                this._setRaycastObjectInfo(this._raycastObject, geometryId, start, count);
                this._raycastObject.raycast(raycaster, this._batchIntersects);
                for (var j = 0, l = this._typedBatchIntersects.length; j < l; j++) {
                    var intersect = this._typedBatchIntersects[j];
        ...
        return AcTrBatchedMixinBase;
    }(Base));
}
//# sourceMappingURL=AcTrBatchedMixin.js.map