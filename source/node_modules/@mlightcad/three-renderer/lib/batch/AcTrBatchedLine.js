...
import { copyArrayContents, isBatchGeometryActive } from './AcTrBatchedGeometryInfo';
import { applyGeometryAt, assertReservedCapacity, createAcTrBatchedMixin, createGeometryState, growCapacityIfNeeded, initializeGeometry, reserveGeometryId, resolveReservedCount, validateGeometry } from './AcTrBatchedMixin';
 ...
 */
var AcTrBatchedLineBase = createAcTrBatchedMixin(THREE.LineSegments, {
    typeName: 'AcTrBatchedLine',
    createObject: function () { return new THREE.LineSegments(); },
    getDrawRange: function (instance, geometryInfo) {
        return instance.geometry.index != null
 ...
 */
var AcTrBatchedLine = /** @class */ (function (_super) {
    __extends(AcTrBatchedLine, _super);
    /**
     ...
     */
    function AcTrBatchedLine(maxVertexCount, maxIndexCount, material) {
        if (maxVertexCount === void 0) { maxVertexCount = 1000; }
        if (maxIndexCount === void 0) { maxIndexCount = maxVertexCount * 2; }
        var _this = _super.call(this, new THREE.BufferGeometry(), material) || this;
    ...
    Object.defineProperty(AcTrBatchedLine.prototype, "geometryCount", {
        /**
    ...
    Object.defineProperty(AcTrBatchedLine.prototype, "unusedVertexCount", {
        /**
    ...
    Object.defineProperty(AcTrBatchedLine.prototype, "unusedIndexCount", {
        /**
    ...
    Object.defineProperty(AcTrBatchedLine.prototype, "origin", {
     ...
     */
    AcTrBatchedLine.prototype._initializeGeometry = function (reference) {
        if (this._geometryInitialized === false) {
            initializeGeometry(this.geometry, reference, this._maxVertexCount, this._maxIndexCount);
     ...
     */
    AcTrBatchedLine.prototype._validateGeometry = function (geometry) {
     ...
     */
    AcTrBatchedLine.prototype._resizeSpaceIfNeeded = function (geometry) {
        var index = geometry.getIndex();
        var newMaxIndexCount = index == null
            ? this._maxIndexCount
        ...
        var newMaxVertexCount = positionAttribute == null
        ...
        if (newMaxIndexCount > this._maxIndexCount ||
     ...
     */
    AcTrBatchedLine.prototype.reset = function () {
     ...
     */
    AcTrBatchedLine.prototype.getUserData = function () {
        var userData = [];
        var geometryInfoList = this._geometryInfo;
        geometryInfoList.forEach(function (item) {
            userData.push({
     ...
     */
    AcTrBatchedLine.prototype.resetGeometry = function (displayMode, displaySize) {
        var _this = this;
        ...
        // Reset states
        this.reset();
        var creator = AcTrPointSymbolCreator.instance;
        userData.forEach(function (item) {
            var _a;
            if (item.position) {
                var geometry = creator.create(displayMode, item.position, displaySize);
                if (geometry.line) {
                    var worldOffset = new THREE.Vector3(item.position.x, item.position.y, (_a = item.position.z) !== null && _a !== void 0 ? _a : 0);
                    var geometryId = _this.addGeometry(geometry.line, -1, -1, worldOffset);
                    _this.setGeometryInfo(geometryId, item);
                }
            }
        });
    };
     ...
     */
    AcTrBatchedLine.prototype.addGeometry = function (geometry, reservedVertexCount, reservedIndexCount, worldOffset) {
        if (reservedVertexCount === void 0) { reservedVertexCount = -1; }
        if (reservedIndexCount === void 0) { reservedIndexCount = -1; }
        ...
        var positionCount = geometry.getAttribute('position').count;
        var index = geometry.getIndex();
        var geometryInfo = __assign({ 
            // geometry information
            vertexStart: this._nextVertexStart, vertexCount: -1, reservedVertexCount: resolveReservedCount(reservedVertexCount, positionCount), indexStart: index ? this._nextIndexStart : -1, indexCount: -1, reservedIndexCount: index
                ? resolveReservedCount(reservedIndexCount, index.count)
                : 0 }, createGeometryState());
     ...
     */
    AcTrBatchedLine.prototype.rebaseGeometryInPlace = function (geometry, worldOffset) {
        ...
        for (var i = 0; i < position.count; i++) {
            position.setXYZ(i, position.getX(i) + worldOffset.x - origin.x, position.getY(i) + worldOffset.y - origin.y, position.getZ(i) + worldOffset.z - origin.z);
        }
        position.needsUpdate = true;
    };
    /**
     ...
     */
    AcTrBatchedLine.prototype.setGeometryInfo = function (geometryId, userData) {
        ...
        if (position)
            geometryInfo.position = __assign({}, position);
     ...
     */
    AcTrBatchedLine.prototype.setGeometryAt = function (geometryId, geometry) {
        if (geometryId >= this._geometryCount) {
            throw new Error('AcTrBatchedLine: Maximum geometry count reached.');
        }
        this._validateGeometry(geometry);
     ...
     */
    AcTrBatchedLine.prototype.optimize = function () {
        var e_1, _a;
        var geometry = this.geometry;
        var hasIndex = geometry.index !== null;
        ...
        try {
            for (var entries_1 = __values(entries), entries_1_1 = entries_1.next(); !entries_1_1.done; entries_1_1 = entries_1.next()) {
                ...
                var indexCount = hasIndex ? info.indexCount : 0;
                var oldVertexStart = info.vertexStart;
                var newVertexStart = nextVertexStart;
                var vertexDelta = newVertexStart - oldVertexStart;
                // ---------- move vertex attributes ----------
                if (vertexDelta !== 0 && vertexCount > 0) {
                    for (var key in attributes) {
                        var attr = attributes[key];
                        var array = attr.array, itemSize = attr.itemSize;
                        array.copyWithin(newVertexStart * itemSize, oldVertexStart * itemSize, (oldVertexStart + vertexCount) * itemSize);
                ...
                if (hasIndex && indexAttr && indexCount > 0) {
                    var oldIndexStart = info.indexStart;
                    var newIndexStart = nextIndexStart;
                    var indexArray = indexAttr.array;
                    // remap vertex indices FIRST
                    if (vertexDelta !== 0) {
                        for (var i = oldIndexStart; i < oldIndexStart + indexCount; i++) {
                            indexArray[i] += vertexDelta;
                        }
                    }
                    // move index range
                    if (oldIndexStart !== newIndexStart) {
                        indexArray.copyWithin(newIndexStart, oldIndexStart, oldIndexStart + indexCount);
                    }
        ...
        if (hasIndex && indexAttr) {
            var indexArray = indexAttr.array;
            for (var i = nextIndexStart; i < indexArray.length; i++) {
                indexArray[i] = 0;
            }
            indexAttr.needsUpdate = true;
        }
     ...
     */
    AcTrBatchedLine.prototype.getBoundingBoxAt = function (geometryId, target) {
        if (geometryId >= this._geometryCount) {
            return null;
        }
        ...
        if (geometryInfo.boundingBox === null) {
            ...
            var _a = index != null
                ? { start: geometryInfo.indexStart, count: geometryInfo.indexCount }
                : { start: geometryInfo.vertexStart, count: geometryInfo.vertexCount }, start = _a.start, count = _a.count;
     ...
     * @returns `target` when the id is valid, otherwise `null`.
     */
    AcTrBatchedLine.prototype.getBoundingSphereAt = function (geometryId, target) {
        if (geometryId >= this._geometryCount) {
            return null;
        }
     ...
     */
    AcTrBatchedLine.prototype.getGeometryAt = function (geometryId) {
     ...
     */
    AcTrBatchedLine.prototype.setGeometrySize = function (maxVertexCount, maxIndexCount) {
     ...
     */
    AcTrBatchedLine.prototype.getObjectAt = function (batchId) {
        var object = _super.prototype.getObjectAt.call(this, batchId);
     ...
     */
    AcTrBatchedLine.prototype._initializeRaycastObject = function (raycastObject) {
        _super.prototype._initializeRaycastObject.call(this, raycastObject);
     ...
     */
    AcTrBatchedLine.prototype._syncDrawRange = function () {
        var geometry = this.geometry;
        if (geometry.index) {
     ...
     * @returns This instance for chaining.
     */
    AcTrBatchedLine.prototype.copy = function (source) {
        var _a;
        ...
        this._geometryInfo = source._geometryInfo.map(function (info) { return (__assign(__assign({}, info), { boundingBox: info.boundingBox !== null ? info.boundingBox.clone() : null })); });
    ...
    AcTrBatchedLine.GROWTH_FACTOR = 1.25;
    return AcTrBatchedLine;
}(AcTrBatchedLineBase));
export { AcTrBatchedLine };
//# sourceMappingURL=AcTrBatchedLine.js.map