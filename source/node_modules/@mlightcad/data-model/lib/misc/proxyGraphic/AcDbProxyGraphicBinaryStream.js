/**
 * Little-endian binary readers for AutoCAD proxy-entity graphics.
 *
 * Proxy-entity graphics combine aligned byte records and DWG-style bit-packed
 * fields. These stream helpers decode both representations and are based on the
 * format documented by ODA and implemented in ezdxf's `ProxyGraphic`.
 */
var __extends = (this && this.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (Object.prototype.hasOwnProperty.call(b, p)) d[p] = b[p]; };
        return extendStatics(d, b);
    };
    return function (d, b) {
        if (typeof b !== "function" && b !== null)
            throw new TypeError("Class extends value " + String(b) + " is not a constructor or null");
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
var __values = (this && this.__values) || function(o) {
    var s = typeof Symbol === "function" && Symbol.iterator, m = s && o[s], i = 0;
    if (m) return m.call(o);
    if (o && typeof o.length === "number") return {
        next: function () {
            if (o && i >= o.length) o = void 0;
            return { value: o && o[i++], done: !o };
        }
    };
    throw new TypeError(s ? "Object is not iterable." : "Symbol.iterator is not defined.");
};
/**
 * Error thrown when a proxy-graphic reader reaches the end of its buffer
 * unexpectedly or cannot locate a required terminator.
 */
var AcDbProxyGraphicEndOfBufferError = /** @class */ (function (_super) {
    __extends(AcDbProxyGraphicEndOfBufferError, _super);
    /**
     * Creates a new end-of-buffer error.
     *
     * @param message - Human-readable error description.
     */
    function AcDbProxyGraphicEndOfBufferError(message) {
        if (message === void 0) { message = 'Unexpected end of buffer.'; }
        var _this = _super.call(this, message) || this;
        _this.name = 'AcDbProxyGraphicEndOfBufferError';
        return _this;
    }
    return AcDbProxyGraphicEndOfBufferError;
}(Error));
export { AcDbProxyGraphicEndOfBufferError };
/**
 * Normalizes an {@link ArrayBuffer} or {@link Uint8Array} to a byte view.
 *
 * @param buffer - Source buffer.
 * @returns A {@link Uint8Array} view over the input bytes.
 */
function toBufferView(buffer) {
    return buffer instanceof ArrayBuffer ? new Uint8Array(buffer) : buffer;
}
/**
 * Sequential little-endian byte reader for aligned proxy-graphic records.
 *
 * Reads fixed-size numeric fields and null-terminated strings from a chunk
 * payload. After each multi-byte read the cursor is advanced to the next
 * 4-byte-aligned offset, matching the ODA proxy-graphic layout.
 */
var AcDbProxyGraphicByteStream = /** @class */ (function () {
    /**
     * Creates a byte stream over the given buffer.
     *
     * @param buffer - Chunk payload bytes.
     * @param align - Alignment size in bytes. Defaults to `4`.
     */
    function AcDbProxyGraphicByteStream(buffer, align) {
        if (align === void 0) { align = 4; }
        /** Current read cursor in bytes. */
        this._index = 0;
        this._buffer = toBufferView(buffer);
        this._align = align;
    }
    Object.defineProperty(AcDbProxyGraphicByteStream.prototype, "index", {
        /**
         * Gets the current read cursor in bytes.
         */
        get: function () {
            return this._index;
        },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(AcDbProxyGraphicByteStream.prototype, "hasData", {
        /**
         * Indicates whether unread bytes remain in the buffer.
         */
        get: function () {
            return this._index < this._buffer.length;
        },
        enumerable: false,
        configurable: true
    });
    /**
     * Rounds a byte index up to the configured alignment boundary.
     *
     * @param index - Raw byte index.
     * @returns The aligned index.
     */
    AcDbProxyGraphicByteStream.prototype.alignIndex = function (index) {
        var modulo = index % this._align;
        return modulo ? index + this._align - modulo : index;
    };
    /**
     * Reads a composite struct of little-endian numeric fields.
     *
     * Supported field sizes are `4` (uint32) and `8` (float64).
     *
     * @typeParam T - Tuple type describing the returned numeric array.
     * @param sizes - Byte width of each field in order.
     * @returns The decoded numeric values.
     * @throws {@link AcDbProxyGraphicEndOfBufferError} When the buffer ends
     *   before the struct can be read.
     * @throws {Error} When an unsupported field size is requested.
     */
    AcDbProxyGraphicByteStream.prototype.readStruct = function (sizes) {
        var e_1, _a;
        if (!this.hasData) {
            throw new AcDbProxyGraphicEndOfBufferError();
        }
        var values = [];
        var byteSize = 0;
        try {
            for (var sizes_1 = __values(sizes), sizes_1_1 = sizes_1.next(); !sizes_1_1.done; sizes_1_1 = sizes_1.next()) {
                var size = sizes_1_1.value;
                var view = new DataView(this._buffer.buffer, this._buffer.byteOffset + this._index + byteSize, size);
                if (size === 8) {
                    values.push(view.getFloat64(0, true));
                }
                else if (size === 4) {
                    values.push(view.getUint32(0, true));
                }
                else {
                    throw new Error("Unsupported struct size: ".concat(size));
                }
                byteSize += size;
            }
        }
        catch (e_1_1) { e_1 = { error: e_1_1 }; }
        finally {
            try {
                if (sizes_1_1 && !sizes_1_1.done && (_a = sizes_1.return)) _a.call(sizes_1);
            }
            finally { if (e_1) throw e_1.error; }
        }
        this._index = this.alignIndex(this._index + byteSize);
        return values;
    };
    /**
     * Reads one little-endian 64-bit floating-point value.
     *
     * @returns The decoded double.
     */
    AcDbProxyGraphicByteStream.prototype.readFloat = function () {
        return this.readStruct([8])[0];
    };
    /**
     * Reads one little-endian 32-bit unsigned integer.
     *
     * @returns The decoded unsigned long.
     */
    AcDbProxyGraphicByteStream.prototype.readLong = function () {
        return this.readStruct([4])[0];
    };
    /**
     * Reads one little-endian 32-bit signed integer.
     *
     * @returns The decoded signed long.
     * @throws {@link AcDbProxyGraphicEndOfBufferError} When the buffer ends
     *   before four bytes are available.
     */
    AcDbProxyGraphicByteStream.prototype.readSignedLong = function () {
        if (!this.hasData) {
            throw new AcDbProxyGraphicEndOfBufferError();
        }
        var view = new DataView(this._buffer.buffer, this._buffer.byteOffset + this._index, 4);
        var value = view.getInt32(0, true);
        this._index = this.alignIndex(this._index + 4);
        return value;
    };
    /**
     * Reads one 3D vertex as three little-endian doubles `(x, y, z)`.
     *
     * @returns The decoded vertex tuple in WCS order.
     */
    AcDbProxyGraphicByteStream.prototype.readVertex = function () {
        return this.readStruct([8, 8, 8]);
    };
    /**
     * Reads a null-terminated single-byte string and advances to the next
     * aligned offset.
     *
     * @param encoding - TextDecoder label for the string bytes. Defaults to
     *   `'utf-8'`.
     * @returns The decoded string without the terminating zero byte.
     * @throws {@link AcDbProxyGraphicEndOfBufferError} When no zero terminator
     *   is found.
     */
    AcDbProxyGraphicByteStream.prototype.readPaddedString = function (encoding) {
        if (encoding === void 0) { encoding = 'utf-8'; }
        var buffer = this._buffer;
        for (var endIndex = this._index; endIndex < buffer.length; endIndex++) {
            if (buffer[endIndex] === 0) {
                var startIndex = this._index;
                this._index = this.alignIndex(endIndex + 1);
                var slice = buffer.subarray(startIndex, endIndex);
                return new TextDecoder(encoding).decode(slice);
            }
        }
        throw new AcDbProxyGraphicEndOfBufferError('Did not detect terminating zero byte.');
    };
    /**
     * Reads a null-terminated UTF-16LE string and advances to the next aligned
     * offset.
     *
     * @returns The decoded Unicode string without the terminating `0x0000`
     *   code unit pair.
     * @throws {@link AcDbProxyGraphicEndOfBufferError} When no zero terminator
     *   is found.
     */
    AcDbProxyGraphicByteStream.prototype.readPaddedUnicodeString = function () {
        var buffer = this._buffer;
        for (var endIndex = this._index; endIndex < buffer.length; endIndex += 2) {
            if (endIndex + 1 < buffer.length &&
                buffer[endIndex] === 0 &&
                buffer[endIndex + 1] === 0) {
                var startIndex = this._index;
                this._index = this.alignIndex(endIndex + 2);
                var slice = buffer.subarray(startIndex, endIndex);
                return new TextDecoder('utf-16le').decode(slice);
            }
        }
        throw new AcDbProxyGraphicEndOfBufferError('Did not detect terminating zero bytes.');
    };
    return AcDbProxyGraphicByteStream;
}());
export { AcDbProxyGraphicByteStream };
/**
 * DWG-style bit-stream reader for compact proxy-graphic payloads.
 *
 * Implements the AutoCAD bit-packing scheme used by lightweight polyline and
 * other compressed proxy-graphic records, including optional default-value
 * doubles and version-dependent fields.
 */
var AcDbProxyGraphicBitStream = /** @class */ (function () {
    /**
     * Creates a bit stream over the given buffer.
     *
     * @param buffer - Chunk payload bytes.
     * @param dxfversion - Drawing version string. Defaults to `'AC1015'`.
     * @param encoding - Single-byte text encoding. Defaults to `'cp1252'`.
     */
    function AcDbProxyGraphicBitStream(buffer, dxfversion, encoding) {
        if (dxfversion === void 0) { dxfversion = 'AC1015'; }
        if (encoding === void 0) { encoding = 'cp1252'; }
        /** Current read cursor in bits. */
        this._bitIndex = 0;
        this._buffer = toBufferView(buffer);
        this.dxfversion = dxfversion;
        this.encoding = encoding;
    }
    Object.defineProperty(AcDbProxyGraphicBitStream.prototype, "hasData", {
        /**
         * Indicates whether unread bits remain in the buffer.
         */
        get: function () {
            return this._bitIndex >> 3 < this._buffer.length;
        },
        enumerable: false,
        configurable: true
    });
    /**
     * Reads a single bit from the stream.
     *
     * @returns `1` when the bit is set, otherwise `0`.
     * @throws {@link AcDbProxyGraphicEndOfBufferError} When no byte is available.
     */
    AcDbProxyGraphicBitStream.prototype.readBit = function () {
        var index = this._bitIndex;
        this._bitIndex += 1;
        var byteIndex = index >> 3;
        if (byteIndex >= this._buffer.length) {
            throw new AcDbProxyGraphicEndOfBufferError();
        }
        return this._buffer[byteIndex] & (0x80 >> (index & 7)) ? 1 : 0;
    };
    /**
     * Reads the next `count` bits as an unsigned integer, MSB first within the
     * field.
     *
     * @param count - Number of bits to read.
     * @returns The assembled unsigned value.
     * @throws {@link AcDbProxyGraphicEndOfBufferError} When the read would pass
     *   the end of the buffer.
     */
    AcDbProxyGraphicBitStream.prototype.readBits = function (count) {
        var index = this._bitIndex;
        var nextBitIndex = index + count;
        if ((nextBitIndex - 1) >> 3 >= this._buffer.length) {
            throw new AcDbProxyGraphicEndOfBufferError();
        }
        this._bitIndex = nextBitIndex;
        var testBit = 0x80 >> (index & 7);
        var testByteIndex = index >> 3;
        var value = 0;
        var testByte = this._buffer[testByteIndex];
        while (count > 0) {
            value <<= 1;
            if (testByte & testBit) {
                value |= 1;
            }
            count -= 1;
            testBit >>= 1;
            if (!testBit && count) {
                testBit = 0x80;
                testByteIndex += 1;
                testByte = this._buffer[testByteIndex];
            }
        }
        return value;
    };
    /**
     * Reads one unsigned 8-bit value from the bit stream.
     *
     * @returns The decoded byte in the range `0..255`.
     */
    AcDbProxyGraphicBitStream.prototype.readUnsignedByte = function () {
        return this.readBits(8);
    };
    /**
     * Reads one signed 8-bit value from the bit stream.
     *
     * @returns The decoded signed byte.
     */
    AcDbProxyGraphicBitStream.prototype.readSignedByte = function () {
        var value = this.readBits(8);
        return value & 0x80 ? -((~value & 0xff) + 1) : value;
    };
    /**
     * Reads `count` whole bytes from the stream, aligning the bit cursor first
     * when currently mid-byte.
     *
     * @param count - Number of bytes to read.
     * @returns A subarray view over the consumed bytes.
     * @throws {@link AcDbProxyGraphicEndOfBufferError} When the read would pass
     *   the end of the buffer.
     */
    AcDbProxyGraphicBitStream.prototype.readAlignedBytes = function (count) {
        var startIndex = this._bitIndex >> 3;
        var endIndex = startIndex + count;
        if (endIndex > this._buffer.length) {
            throw new AcDbProxyGraphicEndOfBufferError();
        }
        this._bitIndex += count << 3;
        return this._buffer.subarray(startIndex, endIndex);
    };
    /**
     * Reads one little-endian unsigned 16-bit integer.
     *
     * @returns The decoded unsigned short.
     */
    AcDbProxyGraphicBitStream.prototype.readUnsignedShort = function () {
        if (this._bitIndex & 7) {
            var s1 = this.readBits(8);
            var s2 = this.readBits(8);
            return (s2 << 8) + s1;
        }
        var bytes = this.readAlignedBytes(2);
        return (bytes[1] << 8) | bytes[0];
    };
    /**
     * Reads one little-endian signed 16-bit integer.
     *
     * @returns The decoded signed short.
     */
    AcDbProxyGraphicBitStream.prototype.readSignedShort = function () {
        var value = this.readUnsignedShort();
        return value & 0x8000 ? -((~value & 0xffff) + 1) : value;
    };
    /**
     * Reads one little-endian unsigned 32-bit integer.
     *
     * @returns The decoded unsigned long.
     */
    AcDbProxyGraphicBitStream.prototype.readUnsignedLong = function () {
        if (this._bitIndex & 7) {
            var l1 = this.readBits(8);
            var l2 = this.readBits(8);
            var l3 = this.readBits(8);
            var l4 = this.readBits(8);
            return (l4 << 24) + (l3 << 16) + (l2 << 8) + l1;
        }
        var bytes = this.readAlignedBytes(4);
        return (bytes[3] << 24) + (bytes[2] << 16) + (bytes[1] << 8) + bytes[0];
    };
    /**
     * Reads one little-endian signed 32-bit integer.
     *
     * @returns The decoded signed long.
     */
    AcDbProxyGraphicBitStream.prototype.readSignedLong = function () {
        var value = this.readUnsignedLong();
        return value & 0x80000000 ? -((~value & 0xffffffff) + 1) : value;
    };
    /**
     * Reads one little-endian 64-bit floating-point value.
     *
     * @returns The decoded double.
     */
    AcDbProxyGraphicBitStream.prototype.readFloat = function () {
        if (this._bitIndex & 7) {
            var data = new Uint8Array(8);
            for (var i = 0; i < 8; i++) {
                data[i] = this.readBits(8);
            }
            return new DataView(data.buffer).getFloat64(0, true);
        }
        var bytes = this.readAlignedBytes(8);
        var copy = new Uint8Array(bytes);
        return new DataView(copy.buffer).getFloat64(0, true);
    };
    /**
     * Reads one or more raw little-endian doubles without bit-short compression.
     *
     * @param count - Number of doubles to read. Defaults to `1`.
     * @returns A single number when `count === 1`, otherwise an array of doubles.
     */
    AcDbProxyGraphicBitStream.prototype.readRawDouble = function (count) {
        var _this = this;
        if (count === void 0) { count = 1; }
        if (count === 1) {
            return this.readFloat();
        }
        return Array.from({ length: count }, function () { return _this.readFloat(); });
    };
    /**
     * Reads one or more DWG **bit-short** values.
     *
     * The 2-bit prefix selects between a full short, a single byte, zero, or the
     * sentinel value `256`.
     *
     * @param count - Number of values to read. Defaults to `1`.
     * @returns A single value or an array of values.
     */
    AcDbProxyGraphicBitStream.prototype.readBitShort = function (count) {
        var _this = this;
        if (count === void 0) { count = 1; }
        var readOne = function () {
            var bits = _this.readBits(2);
            if (bits === 0)
                return _this.readSignedShort();
            if (bits === 1)
                return _this.readUnsignedByte();
            if (bits === 2)
                return 0;
            return 256;
        };
        if (count === 1)
            return readOne();
        return Array.from({ length: count }, function () { return readOne(); });
    };
    /**
     * Reads one or more DWG **bit-long** values.
     *
     * @param count - Number of values to read. Defaults to `1`.
     * @returns A single value or an array of values.
     */
    AcDbProxyGraphicBitStream.prototype.readBitLong = function (count) {
        var _this = this;
        if (count === void 0) { count = 1; }
        var readOne = function () {
            var bits = _this.readBits(2);
            if (bits === 0)
                return _this.readSignedLong();
            if (bits === 1)
                return _this.readUnsignedByte();
            if (bits === 2)
                return 0;
            return 256;
        };
        if (count === 1)
            return readOne();
        return Array.from({ length: count }, function () { return readOne(); });
    };
    /**
     * Reads one or more DWG **bit-double** values.
     *
     * @param count - Number of values to read. Defaults to `1`.
     * @returns A single value or an array of values.
     */
    AcDbProxyGraphicBitStream.prototype.readBitDouble = function (count) {
        var _this = this;
        if (count === void 0) { count = 1; }
        var readOne = function () {
            var bits = _this.readBits(2);
            if (bits === 0)
                return _this.readFloat();
            if (bits === 1)
                return 1;
            if (bits === 2)
                return 0;
            return 0;
        };
        if (count === 1)
            return readOne();
        return Array.from({ length: count }, function () { return readOne(); });
    };
    /**
     * Reads one or more DWG **default bit-double** values.
     *
     * When the prefix indicates "default", `defaultValue` is returned unchanged.
     * Otherwise a partial or full double is reconstructed from the stream.
     *
     * @param count - Number of values to read. Defaults to `1`.
     * @param defaultValue - Fallback double used for default-encoded values.
     * @returns A single value or an array of values.
     */
    AcDbProxyGraphicBitStream.prototype.readBitDoubleDefault = function (count, defaultValue) {
        var _this = this;
        if (count === void 0) { count = 1; }
        if (defaultValue === void 0) { defaultValue = 0; }
        var defaultBytes = new Uint8Array(8);
        new DataView(defaultBytes.buffer).setFloat64(0, defaultValue, true);
        var readOne = function () {
            var bits = _this.readBits(2);
            if (bits === 0)
                return defaultValue;
            if (bits === 1) {
                var data = new Uint8Array(defaultBytes);
                data[0] = _this.readUnsignedByte();
                data[1] = _this.readUnsignedByte();
                data[2] = _this.readUnsignedByte();
                data[3] = _this.readUnsignedByte();
                return new DataView(data.buffer).getFloat64(0, true);
            }
            if (bits === 2) {
                var data = new Uint8Array(defaultBytes);
                data[4] = _this.readUnsignedByte();
                data[5] = _this.readUnsignedByte();
                data[0] = _this.readUnsignedByte();
                data[1] = _this.readUnsignedByte();
                data[2] = _this.readUnsignedByte();
                data[3] = _this.readUnsignedByte();
                return new DataView(data.buffer).getFloat64(0, true);
            }
            return _this.readFloat();
        };
        if (count === 1)
            return readOne();
        return Array.from({ length: count }, function () { return readOne(); });
    };
    return AcDbProxyGraphicBitStream;
}());
export { AcDbProxyGraphicBitStream };
/**
 * Converts a byte array to an upper-case hexadecimal string.
 *
 * Used when writing proxy graphics to DXF group code **310** chunks.
 *
 * @param data - Bytes to encode.
 * @returns A contiguous hex string with two digits per byte.
 */
export function acdbBytesToHexString(data) {
    return Array.from(data, function (byte) { return byte.toString(16).padStart(2, '0'); })
        .join('')
        .toUpperCase();
}
/**
 * Decodes one or more hexadecimal strings into a single byte array.
 *
 * Non-hex characters are not filtered; each pair of characters is parsed with
 * `parseInt(..., 16)`.
 *
 * @param chunks - Hex strings, typically from DXF group code **310**.
 * @returns Concatenated decoded bytes.
 */
export function acdbHexStringsToBytes(chunks) {
    var e_2, _a;
    var totalLength = chunks.reduce(function (sum, chunk) { return sum + Math.floor(chunk.length / 2); }, 0);
    var result = new Uint8Array(totalLength);
    var offset = 0;
    try {
        for (var chunks_1 = __values(chunks), chunks_1_1 = chunks_1.next(); !chunks_1_1.done; chunks_1_1 = chunks_1.next()) {
            var chunk = chunks_1_1.value;
            for (var i = 0; i < chunk.length; i += 2) {
                result[offset++] = parseInt(chunk.slice(i, i + 2), 16);
            }
        }
    }
    catch (e_2_1) { e_2 = { error: e_2_1 }; }
    finally {
        try {
            if (chunks_1_1 && !chunks_1_1.done && (_a = chunks_1.return)) _a.call(chunks_1);
        }
        finally { if (e_2) throw e_2.error; }
    }
    return result;
}
/**
 * Combines DXF group-code **310** payload chunks that may arrive either as
 * hexadecimal strings (ASCII DXF readers that leave hex raw) or as already
 * decoded binary (`Uint8Array` from typed/binary pair readers).
 *
 * Calling `String(Uint8Array)` corrupts the payload (comma-separated decimals),
 * so callers must use this helper instead of treating every chunk as hex text.
 *
 * @param chunks - Ordered group **310** values from DXF.
 * @returns Concatenated bytes in chunk order.
 */
export function acdbCombineDxfBinaryChunks(chunks) {
    var e_3, _a, e_4, _b;
    if (chunks.length === 0) {
        return new Uint8Array();
    }
    var parts = [];
    try {
        for (var chunks_2 = __values(chunks), chunks_2_1 = chunks_2.next(); !chunks_2_1.done; chunks_2_1 = chunks_2.next()) {
            var chunk = chunks_2_1.value;
            if (chunk instanceof Uint8Array) {
                parts.push(chunk);
            }
            else {
                parts.push(acdbHexStringsToBytes([String(chunk)]));
            }
        }
    }
    catch (e_3_1) { e_3 = { error: e_3_1 }; }
    finally {
        try {
            if (chunks_2_1 && !chunks_2_1.done && (_a = chunks_2.return)) _a.call(chunks_2);
        }
        finally { if (e_3) throw e_3.error; }
    }
    var totalLength = parts.reduce(function (sum, part) { return sum + part.length; }, 0);
    var result = new Uint8Array(totalLength);
    var offset = 0;
    try {
        for (var parts_1 = __values(parts), parts_1_1 = parts_1.next(); !parts_1_1.done; parts_1_1 = parts_1.next()) {
            var part = parts_1_1.value;
            result.set(part, offset);
            offset += part.length;
        }
    }
    catch (e_4_1) { e_4 = { error: e_4_1 }; }
    finally {
        try {
            if (parts_1_1 && !parts_1_1.done && (_b = parts_1.return)) _b.call(parts_1);
        }
        finally { if (e_4) throw e_4.error; }
    }
    return result;
}
//# sourceMappingURL=AcDbProxyGraphicBinaryStream.js.map