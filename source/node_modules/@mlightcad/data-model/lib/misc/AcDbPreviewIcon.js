 ...
 */
export function acdbPreviewIconToDataUrl(data) {
    if (!data || data.length === 0)
        return undefined;
    var bmpBytes = ensureBmpFileBytes(data);
    if (!bmpBytes)
        return undefined;
 ...
 */
export function acdbThumbnailImageToDataUrl(data) {
    if (!data || data.length === 0)
        return undefined;
...
function bytesToDataUrl(data, mime) {
    ...
    return "data:".concat(mime, ";base64,").concat(btoa(binary));
}
/**
 ...
 */
function ensureBmpFileBytes(data) {
    ...
    var biSize = view.getUint32(0, true);
    if (biSize < 12 || biSize > data.length)
        return undefined;
    var biBitCount = view.getUint16(14, true);
    var biClrUsed = view.getUint32(32, true);
    var paletteEntries = biBitCount <= 8
        ? biClrUsed > 0
            ? biClrUsed
            : 1 << biBitCount
        : biClrUsed > 0
    ...
    var paletteBytes = biSize === 12 ? paletteEntries * 3 : paletteEntries * 4;
    var pixelOffset = 14 + biSize + paletteBytes;
    if (pixelOffset > 14 + data.length)
        return undefined;
    var fileSize = 14 + data.length;
    var header = new Uint8Array(14);
...
//# sourceMappingURL=AcDbPreviewIcon.js.map