...
export var ACDB_OLE_METAFILE_WMF_MIME = 'image/wmf';
/** MIME type used when an OLE presentation stores an Enhanced Metafile (EMF). */
export var ACDB_OLE_METAFILE_EMF_MIME = 'image/emf';
/** Aldus placeable WMF key (`0x9AC6CDD7`). */
var ALDUS_PLACEABLE_KEY = 0x9ac6cdd7;
/**
 * Returns true when `mime` identifies a WMF/EMF blob produced by the OLE
 * image extractor.
 */
export function acdbIsOleMetafileMimeType(mime) {
    return (mime === ACDB_OLE_METAFILE_WMF_MIME || mime === ACDB_OLE_METAFILE_EMF_MIME);
}
/**
 * Detects a standard or Aldus-placeable WMF header at the start of `data`.
 */
export function acdbLooksLikeWmf(data) {
    if (data.length < 18) {
        return false;
    }
 ...
 */
export function acdbReassembleEmfFromWmfEscapes(data) {
    var e_1, _a;
    ...
    var headerSizeWords = view.getUint16(offset + 2, true);
    ...
    while (offset + 6 <= data.length) {
        var recordSizeWords = view.getUint32(offset, true);
        var recordSize = recordSizeWords * 2;
        var recordFunction = view.getUint16(offset + 4, true);
        ...
        if (recordFunction === META_ESCAPE && offset + 44 <= offset + recordSize) {
            var escapeFunction = view.getUint16(offset + 6, true);
            if (escapeFunction === META_ESCAPE_ENHANCED_METAFILE) {
                var commentId = view.getUint32(offset + 10, true);
                // CurrentRecordSize / EnhancedMetafileDataSize (must match per MS-WMF).
                var currentRecordSize = view.getUint32(offset + 32, true);
                var emfDataSize = view.getUint32(offset + 40, true);
                var payloadSize = currentRecordSize > 0 && currentRecordSize === emfDataSize
                    ? currentRecordSize
                    : currentRecordSize > 0
                        ? currentRecordSize
                        : emfDataSize;
                var dataStart = offset + 44;
                if (commentId === WMFC_COMMENT_IDENTIFIER &&
                    payloadSize > 0 &&
    ...
    var emf = new Uint8Array(totalBytes);
    var writeAt = 0;
    try {
 ...
 */
export function findEmfHeaderOffset(data) {
    ...
    var limit = Math.min(data.length - 44, 4096);
    for (var i = 0; i <= limit; i++) {
        if (!isEmfHeaderAt(data, i))
            continue;
        return i;
    }
    return -1;
 ...
 */
export function isEmfHeaderAt(data, offset) {
    if (offset + 88 > data.length) {
        return false;
    }
    var view = new DataView(data.buffer, data.byteOffset + offset, data.length - offset);
    ...
    if (data[offset + 40] !== 0x20 ||
        data[offset + 41] !== 0x45 ||
        data[offset + 42] !== 0x4d ||
        data[offset + 43] !== 0x46) {
        return false;
    }
    var nBytes = view.getUint32(48, true);
    var nRecords = view.getUint32(52, true);
    if (nBytes < headerSize || offset + nBytes > data.length) {
        return false;
    }
    if (nRecords < 2 || nRecords > 10000000) {
        return false;
    }
    return true;
}
function isStandardWmfHeader(data) {
    if (data.length < 18) {
        return false;
    }
    var view = new DataView(data.buffer, data.byteOffset, data.byteLength);
    var type = view.getUint16(0, true);
    var headerSize = view.getUint16(2, true);
    var version = view.getUint16(4, true);
    var sizeWords = view.getUint32(6, true);
...
//# sourceMappingURL=AcDbOleMetafileDetect.js.map