...
import { ACDB_COMPAREHATCH_DEFAULT, ACDB_COMPAREHATCH_MAX, ACDB_COMPAREHATCH_MIN, ACDB_COMPAREPROPS_DEFAULT, ACDB_COMPAREPROPS_MAX, ACDB_COMPAREPROPS_MIN, ACDB_COMPARERCMARGIN_DEFAULT, ACDB_COMPARERCMARGIN_MAX, ACDB_COMPARERCMARGIN_MIN, ACDB_COMPARETEXT_DEFAULT, ACDB_COMPARETEXT_MAX, ACDB_COMPARETEXT_MIN, ACDB_COMPARETOLERANCE_DEFAULT, ACDB_COMPARETOLERANCE_MAX, ACDB_COMPARETOLERANCE_MIN, ACDB_GRIPCOLOR_DEFAULT, ACDB_GRIPCOLOR_MAX, ACDB_GRIPCOLOR_MIN, ACDB_GRIPHOT_DEFAULT, ACDB_GRIPHOT_MAX, ACDB_GRIPHOT_MIN, ACDB_GRIPSIZE_DEFAULT, ACDB_GRIPSIZE_MAX, ACDB_GRIPSIZE_MIN, acdbCoerceIntegerSysVar, ByLayer, DEFAULT_HATCH_PATTERN_METRIC, DEFAULT_MLEADER_STYLE, DEFAULT_MLINE_STYLE, DEFAULT_TEXT_STYLE } from '../misc/AcDbConstants';
 ...
 */
function detectDefaultLoginName() {
    try {
        var env = typeof process !== 'undefined' && process.env ? process.env : undefined;
        if (!env)
            return '';
        var name_1 = env.USERNAME || env.USER || env.LOGNAME || '';
        return typeof name_1 === 'string' ? name_1.trim() : '';
    }
    catch (_a) {
        return '';
    }
...
var AcDbSysVarManager = /** @class */ (function () {
    ...
    function AcDbSysVarManager() {
        ...
        this.registerVar({
            ...
            defaultValue: (function () {
                var c = new AcCmColor(AcCmColorMethod.ByColor);
                c.setRGB(96, 165, 250);
                return c;
            })()
        });
    ...
    AcDbSysVarManager.prototype.registerVar = function (desc) {
        var name = this.normalizeName(desc.name);
        this.registry.set(name, __assign(__assign({}, desc), { name: name }));
        if (!desc.isDbVar) {
     ...
     */
    AcDbSysVarManager.prototype.setLoginName = function (value) {
        var name = this.normalizeName(AcDbSystemVariables.LOGINNAME);
        this.cache.set(name, value.trim());
    };
    /**
     * Get system variable value.
     */
    AcDbSysVarManager.prototype.getVar = function (name, db) {
     ...
     */
    AcDbSysVarManager.prototype.getDefaultValue = function (name) {
        name = this.normalizeName(name);
        var descriptor = this.getDescriptor(name);
        if (!descriptor) {
            throw new Error("System variable ".concat(name, " not found!"));
     ...
     */
    AcDbSysVarManager.prototype.applyVarMutation = function (name, oldVal, newVal, db, mutate) {
        var normalizedName = this.normalizeName(name);
        if (!this.hasValueChanged(oldVal, newVal)) {
            return;
        }
        if (db.transactionManager.isRecording()) {
            db.transactionManager.recordSysvar(normalizedName, oldVal);
        }
        else if (db.transactionManager.strictMode &&
            !db.transactionManager.isApplyingUndoRedo()) {
            throw new Error("Cannot change system variable ".concat(normalizedName, " outside an active transaction."));
        }
        mutate();
        if (!db.transactionManager.isApplyingUndoRedo() &&
     ...
     */
    AcDbSysVarManager.prototype.setVar = function (name, value, db) {
        var _this = this;
        name = this.normalizeName(name);
        var descriptor = this.getDescriptor(name);
        if (descriptor) {
            if (descriptor.readOnly) {
                throw new Error("System variable ".concat(name, " is read-only!"));
            }
            var oldVal = this.getVar(name, db);
            if (descriptor.type === 'transparency') {
                var tmp = this.parseTransparency(value);
                ...
                if (descriptor.type === 'number') {
                    ...
                    var tmp = this.parseColorSysVar(name, value, db);
                    if (tmp == null) {
            ...
            if (name === AcDbSystemVariables.GRIPCOLOR.toLowerCase()) {
                ...
                if (!Number.isFinite(intVal) ||
                    ...
                    throw new Error("Invalid GRIPCOLOR value! Valid range is ".concat(ACDB_GRIPCOLOR_MIN, " to ").concat(ACDB_GRIPCOLOR_MAX, "."));
                }
                value = intVal;
            }
            if (name === AcDbSystemVariables.GRIPHOT.toLowerCase()) {
                ...
                if (!Number.isFinite(intVal) ||
                    ...
                    throw new Error("Invalid GRIPHOT value! Valid range is ".concat(ACDB_GRIPHOT_MIN, " to ").concat(ACDB_GRIPHOT_MAX, "."));
                }
                value = intVal;
            }
            ...
            if (name === AcDbSystemVariables.GRIPSIZE.toLowerCase()) {
                ...
                if (!Number.isFinite(intVal) ||
                    ...
                    throw new Error("Invalid GRIPSIZE value! Valid range is ".concat(ACDB_GRIPSIZE_MIN, " to ").concat(ACDB_GRIPSIZE_MAX, "."));
                }
                value = intVal;
            }
            if (name === AcDbSystemVariables.COMPAREHATCH.toLowerCase()) {
                value = acdbCoerceIntegerSysVar('COMPAREHATCH', value, ACDB_COMPAREHATCH_MIN, ACDB_COMPAREHATCH_MAX);
            }
            if (name === AcDbSystemVariables.COMPAREPROPS.toLowerCase()) {
                value = acdbCoerceIntegerSysVar('COMPAREPROPS', value, ACDB_COMPAREPROPS_MIN, ACDB_COMPAREPROPS_MAX);
            }
            if (name === AcDbSystemVariables.COMPARERCMARGIN.toLowerCase()) {
                value = acdbCoerceIntegerSysVar('COMPARERCMARGIN', value, ACDB_COMPARERCMARGIN_MIN, ACDB_COMPARERCMARGIN_MAX);
            }
            if (name === AcDbSystemVariables.COMPARETEXT.toLowerCase()) {
                value = acdbCoerceIntegerSysVar('COMPARETEXT', value, ACDB_COMPARETEXT_MIN, ACDB_COMPARETEXT_MAX);
            }
            if (name === AcDbSystemVariables.COMPARETOLERANCE.toLowerCase()) {
                value = acdbCoerceIntegerSysVar('COMPARETOLERANCE', value, ACDB_COMPARETOLERANCE_MIN, ACDB_COMPARETOLERANCE_MAX);
            }
            if (descriptor.isDbVar) {
                this.applyVarMutation(name, oldVal, value, db, function () {
                    ;
                    db[name.toLowerCase()] =
                        value;
                });
    ...
    AcDbSysVarManager.prototype.parseColorSysVar = function (name, value, db) {
        var normalized = value.trim().toLowerCase();
        if (name === AcDbSystemVariables.HPCOLOR.toLowerCase() &&
            (normalized === '.' || normalized === 'use current')) {
            return db.cecolor.clone();
        }
        if (name === AcDbSystemVariables.HPBACKGROUNDCOLOR.toLowerCase() &&
            (normalized === '' || normalized === '.' || normalized === 'none')) {
            return new AcCmColor(AcCmColorMethod.None);
        }
        return AcCmColor.fromString(value);
    };
    AcDbSysVarManager.prototype.parseTransparency = function (value) {
     ...
     */
    AcDbSysVarManager.prototype.hasValueChanged = function (oldValue, newValue) {
        if (oldValue instanceof AcCmColor && newValue instanceof AcCmColor) {
            return !oldValue.equals(newValue);
        }
        if (oldValue instanceof AcCmTransparency &&
     ...
     */
    AcDbSysVarManager.prototype.normalizeName = function (name) {
