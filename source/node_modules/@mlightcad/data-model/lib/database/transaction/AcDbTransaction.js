...
var AcDbTransaction = /** @class */ (function () {
    function AcDbTransaction() {
        ...
        this.recorder = new AcDbChangeRecorder();
    }
    /**
    ...
    AcDbTransaction.prototype.getObject = function (objectId, mode, openErased) {
        ...
        var opened = this.openedObjects.get(objectId);
        if (opened) {
            if (mode === AcDbOpenMode.kForWrite &&
                !this.originalStates.has(objectId)) {
                var snapshot = opened.clonePreservingIdentity();
        ...
        if (mode === AcDbOpenMode.kForWrite) {
            var snapshot = obj.clonePreservingIdentity();
