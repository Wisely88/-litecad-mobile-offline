 ...
 */
export var AcDbBlockTableRecordFlag;
(function (AcDbBlockTableRecordFlag) {
    /** No special block type flags apply */
    AcDbBlockTableRecordFlag[AcDbBlockTableRecordFlag["None"] = 0] = "None";
    /** Anonymous block (hatch, associative dimension, etc.) */
    AcDbBlockTableRecordFlag[AcDbBlockTableRecordFlag["Anonymous"] = 1] = "Anonymous";
    /** Block has non-constant attribute definitions */
    AcDbBlockTableRecordFlag[AcDbBlockTableRecordFlag["HasNonConstantAttributes"] = 2] = "HasNonConstantAttributes";
    /** External reference (xref) */
    AcDbBlockTableRecordFlag[AcDbBlockTableRecordFlag["Xref"] = 4] = "Xref";
    /** Xref overlay */
    AcDbBlockTableRecordFlag[AcDbBlockTableRecordFlag["XrefOverlay"] = 8] = "XrefOverlay";
    /** Externally dependent */
    AcDbBlockTableRecordFlag[AcDbBlockTableRecordFlag["ExternallyDependent"] = 16] = "ExternallyDependent";
    /** Resolved external reference (or dependent of one) */
    AcDbBlockTableRecordFlag[AcDbBlockTableRecordFlag["Resolved"] = 32] = "Resolved";
    /** Referenced external reference */
    AcDbBlockTableRecordFlag[AcDbBlockTableRecordFlag["Referenced"] = 64] = "Referenced";
})(AcDbBlockTableRecordFlag || (AcDbBlockTableRecordFlag = {}));
var AcDbBlockTableRecord = /** @class */ (function (_super) {
     ...
     * ```
     */
    function AcDbBlockTableRecord(attrs, defaultAttrs) {
        var _this = this;
        attrs = attrs || {};
        defaults(attrs, {
     ...
     */
    AcDbBlockTableRecord.sanitizeImportedFlags = function (flags) {
        return (flags &
    ...
    Object.defineProperty(AcDbBlockTableRecord.prototype, "flags", {
        /**
    ...
    Object.defineProperty(AcDbBlockTableRecord.prototype, "pathName", {
        /**
    ...
    Object.defineProperty(AcDbBlockTableRecord.prototype, "isXref", {
        /**
    ...
    Object.defineProperty(AcDbBlockTableRecord.prototype, "isOverlayReference", {
        /**
    ...
    Object.defineProperty(AcDbBlockTableRecord.prototype, "isUnresolvedXref", {
        /**
    ...
    Object.defineProperty(AcDbBlockTableRecord.prototype, "previewIcon", {
        /**
    ...
    AcDbBlockTableRecord.prototype.appendEntity = function (entity) {
        var e_1, _a, e_2, _b;
        var _this = this;
        var manager = this.database.transactionManager;
        ...
        var commitEntity = function (item) {
            ...
            _this.database.ensureEntityStyleDefaults(item);
            _this.database.commitObjectHandle(item, function (id) { return _this.hasEntityId(id); });
            item.resolveEffectiveProperties();
            _this._entities.push(item);
            if (item.dxfTypeName === 'INSERT' &&
                'syncAttributeDatabases' in item &&
                typeof item.syncAttributeDatabases === 'function') {
                item.syncAttributeDatabases();
            }
        };
        var entitiesToAppend = Array.isArray(entity) ? entity : [entity];
        try {
            for (var entitiesToAppend_1 = __values(entitiesToAppend), entitiesToAppend_1_1 = entitiesToAppend_1.next(); !entitiesToAppend_1_1.done; entitiesToAppend_1_1 = entitiesToAppend_1.next()) {
                var item = entitiesToAppend_1_1.value;
                commitEntity(item);
            }
        }
            ...
            try {
                if (entitiesToAppend_1_1 && !entitiesToAppend_1_1.done && (_a = entitiesToAppend_1.return)) _a.call(entitiesToAppend_1);
            }
            finally { if (e_1) throw e_1.error; }
        }
        if (manager.isRecording()) {
            try {
                for (var entitiesToAppend_2 = __values(entitiesToAppend), entitiesToAppend_2_1 = entitiesToAppend_2.next(); !entitiesToAppend_2_1.done; entitiesToAppend_2_1 = entitiesToAppend_2.next()) {
                    var item = entitiesToAppend_2_1.value;
                    manager.recordAppend({ type: 'blockTableRecord', ownerId: this.objectId }, item);
                }
            }
            catch (e_2_1) { e_2 = { error: e_2_1 }; }
            finally {
                try {
                    if (entitiesToAppend_2_1 && !entitiesToAppend_2_1.done && (_b = entitiesToAppend_2.return)) _b.call(entitiesToAppend_2);
                }
     ...
     */
    AcDbBlockTableRecord.prototype.removeEntity = function (objectId) {
        var e_3, _a;
        var manager = this.database.transactionManager;
        if (manager.strictMode &&
            !manager.isRecording() &&
        ...
        var ids = Array.isArray(objectId) ? objectId : [objectId];
        if (ids.length === 0) {
            return false;
        }
        ...
        try {
            for (var _b = __values(this._entities), _c = _b.next(); !_c.done; _c = _b.next()) {
                ...
                if (idSet.has(entity.objectId)) {
                    if (manager.isRecording()) {
                        manager.recordRemove({ type: 'blockTableRecord', ownerId: this.objectId }, entity);
                    }
    ...
    AcDbBlockTableRecord.prototype.getIdAt = function (id) {
        var object = this.database.getObjectById(id);
        if (!(object instanceof AcDbEntity)) {
            return undefined;
        }
        var ownerId = object.getAttrWithoutException('ownerId');
     ...
     * @internal
     */
    AcDbBlockTableRecord.prototype.hasEntityId = function (id) {
    ...
    AcDbBlockTableRecord.prototype.dxfInFields = function (filer) {
        ...
        while (!filer.atEndOfObject && !filer.atEof && !filer.atExtendedData) {
            ...
            switch (code) {
                ...
                case 280:
                    ...
                    break;
                case 281:
                    this.blockScaling = Number(item.value);
                    break;
                case 340:
                    this.layoutId = String(item.value);
                    break;
                case 310: {
                    var hex = String(item.value).replace(/\s+/g, '');
                    if (hex)
                        previewChunks.push(hex);
                    break;
                }
                default:
                    break;
