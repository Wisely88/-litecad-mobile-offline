...
var MAX_VIEW_TO_EXTENT_RATIO = 2;
/** Max distance from drawing-extent center as a fraction of the larger span. */
var MAX_VIEW_CENTER_OFFSET_RATIO = 0.45;
/** Max model-space view span when EXTMIN/EXTMAX are unavailable. */
var MAX_VIEW_SPAN_WITHOUT_EXTENTS = 1e5;
function readVportAspectRatio(vport) {
    var _a, _b;
    var source = vport;
    var aspectRatio = (_a = source.aspectRatio) !== null && _a !== void 0 ? _a : (_b = source.gsView) === null || _b === void 0 ? void 0 : _b.aspectRatio;
    return Number.isFinite(aspectRatio) ? aspectRatio : undefined;
}
function resolveViewAspectRatio(vport, canvasAspectRatio) {
    if (Number.isFinite(canvasAspectRatio) && canvasAspectRatio > 0) {
        return canvasAspectRatio;
    }
    var storedAspect = readVportAspectRatio(vport);
    if (storedAspect != null && storedAspect > 0) {
        return storedAspect;
    }
    return 1;
}
function intersectionArea(viewBox, drawingExtents) {
    var minX = Math.max(viewBox.min.x, drawingExtents.min.x);
    var minY = Math.max(viewBox.min.y, drawingExtents.min.y);
    var maxX = Math.min(viewBox.max.x, drawingExtents.max.x);
    var maxY = Math.min(viewBox.max.y, drawingExtents.max.y);
    if (minX >= maxX || minY >= maxY) {
        return 0;
    }
    return (maxX - minX) * (maxY - minY);
}
function viewCenter(viewBox) {
    return {
        x: (viewBox.min.x + viewBox.max.x) / 2,
        y: (viewBox.min.y + viewBox.max.y) / 2
    };
}
function hasMeaningfulDrawingExtents(drawingExtents) {
    if (!drawingExtents || drawingExtents.isEmpty()) {
        return false;
    }
    var spanX = drawingExtents.max.x - drawingExtents.min.x;
    var spanY = drawingExtents.max.y - drawingExtents.min.y;
...
var AcDbViewportTableRecord = /** @class */ (function (_super) {
     ...
     */
    function AcDbViewportTableRecord(attrs, defaultAttrs) {
        attrs = attrs || {};
        defaults(attrs, {
            circleSides: 100,
     ...
     */
    AcDbViewportTableRecord.isActiveVportName = function (name) {
    ...
    Object.defineProperty(AcDbViewportTableRecord.prototype, "ucsElevation", {
        /**
     ...
     */
    AcDbViewportTableRecord.prototype.modelViewBox = function (canvasAspectRatio) {
        ...
        var target = this.viewTarget;
        if (target && Number.isFinite(target.x) && Number.isFinite(target.y)) {
            wcsCenterX += target.x;
            wcsCenterY += target.y;
        }
        var aspectRatio = resolveViewAspectRatio(this, canvasAspectRatio);
     ...
     */
    AcDbViewportTableRecord.isModelViewBoxUsable = function (viewBox, drawingExtents) {
        if (viewBox.isEmpty() || drawingExtents.isEmpty()) {
            return false;
        }
        var viewSpanX = viewBox.max.x - viewBox.min.x;
        var viewSpanY = viewBox.max.y - viewBox.min.y;
        var extSpanX = drawingExtents.max.x - drawingExtents.min.x;
        var extSpanY = drawingExtents.max.y - drawingExtents.min.y;
        if (viewSpanX <= 0 || viewSpanY <= 0 || extSpanX <= 0 || extSpanY <= 0) {
            return false;
        }
        if (viewSpanX > extSpanX * MAX_VIEW_TO_EXTENT_RATIO ||
            viewSpanY > extSpanY * MAX_VIEW_TO_EXTENT_RATIO) {
            return false;
        }
        var centerOffsetLimit = Math.max(extSpanX, extSpanY) * MAX_VIEW_CENTER_OFFSET_RATIO;
        var viewCenterPoint = viewCenter(viewBox);
        var extentCenterPoint = viewCenter(drawingExtents);
        var centerDistance = Math.hypot(viewCenterPoint.x - extentCenterPoint.x, viewCenterPoint.y - extentCenterPoint.y);
        if (centerDistance > centerOffsetLimit) {
            return false;
        }
        var viewArea = viewSpanX * viewSpanY;
        var overlap = intersectionArea(viewBox, drawingExtents);
     ...
     */
    AcDbViewportTableRecord.prototype.resolveModelViewBox = function (canvasAspectRatio, drawingExtents) {
        var viewBox = this.modelViewBox(canvasAspectRatio);
        if (!viewBox ||
    ...
    AcDbViewportTableRecord.isModelViewBoxStructurallyValid = function (viewBox) {
        if (viewBox.isEmpty()) {
            return false;
        }
        var viewSpanX = viewBox.max.x - viewBox.min.x;
        var viewSpanY = viewBox.max.y - viewBox.min.y;
        return (Number.isFinite(viewSpanX) &&
            Number.isFinite(viewSpanY) &&
            viewSpanX > 0 &&
            viewSpanY > 0);
    };
    AcDbViewportTableRecord.isModelViewBoxPlausibleWithoutExtents = function (viewBox) {
        var viewSpanX = viewBox.max.x - viewBox.min.x;
        var viewSpanY = viewBox.max.y - viewBox.min.y;
     ...
     */
    AcDbViewportTableRecord.prototype.dxfOutFields = function (filer) {
        ...
        if (this.gsView.aspectRatio != null &&
    ...
    AcDbViewportTableRecord.prototype.dxfInFields = function (filer) {
        var _a;
        ...
        var frozenLayers = __spreadArray([], __read(((_a = this.gsView.frozenLayers) !== null && _a !== void 0 ? _a : [])), false);
