...
var AcDbDiametricDimension = /** @class */ (function (_super) {
    ...
    Object.defineProperty(AcDbDiametricDimension.prototype, "properties", {
        get: function () {
            var _this = this;
            ...
            return {
                ...
                groups: __spreadArray(__spreadArray([], __read(baseProperties.groups), false), [
                    {
                        groupName: 'geometry',
                        properties: __spreadArray(__spreadArray(__spreadArray([], __read(this.createPoint3dProperties('chordPoint', function () { return _this.chordPoint; })), false), __read(this.createPoint3dProperties('farChordPoint', function () { return _this.farChordPoint; })), false), [
     ...
     * @inheritdoc
     */
    AcDbDiametricDimension.prototype.collectDimensionDefinitionGripPoints = function () {
        return [this.chordPoint, this.farChordPoint];
    };
    /**
     * @inheritdoc
     */
    AcDbDiametricDimension.prototype.subTransformBy = function (matrix) {
        this._chordPoint.applyMatrix4(matrix);
        this._farChordPoint.applyMatrix4(matrix);
    };
    AcDbDiametricDimension.prototype.getMeasurementPropertyValue = function () {
        var _a;
        return (_a = this.measurement) !== null && _a !== void 0 ? _a : this.chordPoint.distanceTo(this.farChordPoint);
    };
    Object.defineProperty(AcDbDiametricDimension.prototype, "geometricExtents", {
         ...
         */
        get: function () {
            var points = [this.chordPoint, this.farChordPoint];
            if (this.leaderLength > 0) {
                var direction = this.farChordPoint.clone().sub(this.chordPoint);
     ...
     */
    AcDbDiametricDimension.prototype.dxfOutFields = function (filer) {
    ...
    AcDbDiametricDimension.prototype.dxfInFields = function (filer) {
        var _this = this;
        ...
        var cx = this._dxfDefinitionPoint.x;
        var cy = this._dxfDefinitionPoint.y;
        var cz = this._dxfDefinitionPoint.z;
        var fx = this.farChordPoint.x;
        var fy = this.farChordPoint.y;
        var fz = this.farChordPoint.z;
        var hasSubDef = false;
        var hasFar = false;
        var startDeg = (this.extArcStartAngle * 180) / Math.PI;
        var endDeg = (this.extArcEndAngle * 180) / Math.PI;
        var commit = function () {
            if (hasFar) {
                // ObjectARX-style: 15 = chord, 16 = far
                _this.chordPoint = new AcGePoint3d(cx, cy, cz);
                _this.farChordPoint = new AcGePoint3d(fx, fy, fz);
            }
            else if (hasSubDef) {
                // AutoCAD DXF / dxf-json-converter: 10 = chord, 15 = far
                _this.chordPoint = new AcGePoint3d(_this._dxfDefinitionPoint.x, _this._dxfDefinitionPoint.y, _this._dxfDefinitionPoint.z);
                _this.farChordPoint = new AcGePoint3d(cx, cy, cz);
            }
            _this.extArcStartAngle = (startDeg * Math.PI) / 180;
            _this.extArcEndAngle = (endDeg * Math.PI) / 180;
        };
