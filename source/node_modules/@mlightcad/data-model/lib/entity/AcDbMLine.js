...
import { ByBlock, ByLayer, DEFAULT_LINE_TYPE, DEFAULT_MLINE_STYLE } from '../misc/AcDbConstants';
...
var AcDbMLine = /** @class */ (function (_super) {
    ...
    /** @inheritdoc */
    AcDbMLine.prototype.subGetIntersectCurves = function () {
        var primitives = [];
        var mlineStyle = this.getMLineStyle();
        var elementCount = this.getRenderableElementCount(mlineStyle);
        if (elementCount <= 0) {
            return acdbIntersectPrimitivesFromPointPath(this.getReferencePath(), this.closed);
        }
        for (var i = 0; i < elementCount; i++) {
            primitives.push.apply(primitives, __spreadArray([], __read(acdbIntersectPrimitivesFromPointPath(this.getElementPath(i, mlineStyle), this.closed)), false));
        }
        return primitives;
    };
    /**
     ...
     */
    AcDbMLine.prototype.subGetGripPoints = function () {
        var gripPoints = new Array();
        gripPoints.push(this._startPosition.clone());
        this._segments.forEach(function (segment) { return gripPoints.push(segment.position.clone()); });
        return gripPoints;
    };
    /** @inheritdoc */
    AcDbMLine.prototype.subMoveGripPointsAt = function (indices, offset) {
        var _this = this;
        var gripPoints = __spreadArray([
            this._startPosition
        ], __read(this._segments.map(function (segment) { return segment.position; })), false);
        acdbMovePointArrayGripAt(indices, offset, gripPoints);
        acdbForEachGripIndex(indices, function (index) {
            _this.updateReferencePathDirectionsAfterGripMove(index);
        });
        return this;
    };
    /**
     ...
     */
    AcDbMLine.prototype.subGetOsnapPoints = function (osnapMode, pickPoint, _lastPoint, snapPoints) {
        var path = __spreadArray([
            this._startPosition
        ], __read(this._segments.map(function (segment) { return segment.position; })), false);
        if (path.length === 0)
            return;
        switch (osnapMode) {
            case AcDbOsnapMode.EndPoint:
                snapPoints.push.apply(snapPoints, __spreadArray([], __read(path), false));
                break;
            ...
            case AcDbOsnapMode.Perpendicular: {
                ...
                for (var index = 0; index < path.length - 1; index++) {
                    var segmentSnaps = [];
                    acdbCollectLineSegmentOsnapPoints(path[index], path[index + 1], osnapMode, pickPoint, segmentSnaps);
    ...
    AcDbMLine.prototype.subWorldDraw = function (renderer) {
        ...
        if (elementCount <= 0) {
            ...
            this.appendStyleDrivenJointAndCapEntities(renderer, entities, mlineStyle, elementCount, originalColor, originalLineType);
        }
        traits.color = originalColor;
        traits.lineType = originalLineType;
        traits.fillType = originalFillType;
    ...
    AcDbMLine.prototype.dxfInFields = function (filer) {
        _super.prototype.dxfInFields.call(this, filer);
        filer.atSubclassData('AcDbMline');
        var sx = this._startPosition.x;
        var sy = this._startPosition.y;
        var sz = this._startPosition.z;
        var nx = this._normal.x;
        var ny = this._normal.y;
        var nz = this._normal.z;
        var segments = [];
        var currentSegment = null;
        var currentElement = null;
        var readingParameters = false;
        var readingFills = false;
        var expectedParams = 0;
        var expectedFills = 0;
        var flushElement = function () {
        ...
        while (!filer.atEndOfObject && !filer.atEof && !filer.atExtendedData) {
            ...
            switch (code) {
                ...
                case 21:
                    if (currentSegment)
                        currentSegment.position.y = n;
                    break;
                case 31:
                    if (currentSegment)
                        currentSegment.position.z = n;
                    break;
                case 12:
                    if (currentSegment)
                        currentSegment.direction.x = n;
                    break;
                case 22:
                    if (currentSegment)
                        currentSegment.direction.y = n;
                    break;
                case 32:
                    if (currentSegment)
                        currentSegment.direction.z = n;
                    break;
                case 13:
                    if (currentSegment)
                        currentSegment.miterDirection.x = n;
                    break;
                case 23:
                    if (currentSegment)
                        currentSegment.miterDirection.y = n;
                    break;
                case 33:
                    if (currentSegment)
                        currentSegment.miterDirection.z = n;
                    break;
                case 74:
                    flushElement();
                    expectedParams = n;
     ...
     */
    AcDbMLine.prototype.updateReferencePathDirectionsAfterGripMove = function (gripIndex) {
        var _this = this;
        var updateDirectionAtSegment = function (segmentIndex) {
            var segment = _this._segments[segmentIndex];
            if (!segment)
                return;
            var previousPoint = segmentIndex === 0
                ? _this._startPosition
                : _this._segments[segmentIndex - 1].position;
            var direction = new AcGeVector3d().subVectors(segment.position, previousPoint);
     ...
     */
    AcDbMLine.prototype.appendStyleDrivenJointAndCapEntities = function (renderer, entities, mlineStyle, elementCount, originalColor, originalLineType) {
        if (!mlineStyle || elementCount < 2)
            return;
        entities.push.apply(entities, __spreadArray([], __read(this.drawMiterJointEntities(renderer, mlineStyle, elementCount, originalColor, originalLineType)), false));
        entities.push.apply(entities, __spreadArray([], __read(this.drawCapEntities(renderer, mlineStyle, elementCount, originalColor, originalLineType)), false));
    };
    /**
     ...
     */
    AcDbMLine.prototype.drawMiterJointEntities = function (renderer, mlineStyle, elementCount, originalColor, originalLineType) {
        var _this = this;
        if (this.closed ||
        ...
        var _loop_1 = function (segmentIndex) {
            ...
            for (var i = 0; i < points.length - 1; i++) {
                this_1.applyElementDrawTraits(traits, originalColor, originalLineType, mlineStyle, points[i].elementIndex);
     ...
     */
    AcDbMLine.prototype.drawCapEntities = function (renderer, mlineStyle, elementCount, originalColor, originalLineType) {
        if (this.closed || this._segments.length <= 0)
            return [];
        var entities = [];
        entities.push.apply(entities, __spreadArray([], __read(this.drawCapEntitiesForSide(renderer, 'start', mlineStyle, elementCount, originalColor, originalLineType)), false));
        entities.push.apply(entities, __spreadArray([], __read(this.drawCapEntitiesForSide(renderer, 'end', mlineStyle, elementCount, originalColor, originalLineType)), false));
        return entities;
    };
    /**
     ...
     */
    AcDbMLine.prototype.drawCapEntitiesForSide = function (renderer, side, mlineStyle, elementCount, originalColor, originalLineType) {
        ...
        if (hasSquare) {
            for (var i = 0; i < points.length - 1; i++) {
                this.applyElementDrawTraits(traits, originalColor, originalLineType, mlineStyle, points[i].elementIndex);
                entities.push(renderer.lines([points[i].point, points[i + 1].point]));
            }
        }
        ...
        if (hasOuterArc) {
            var outer = this.drawCapArcBetweenElements(renderer, side, points[0], points[points.length - 1], capDirection, traits, originalColor, originalLineType, mlineStyle);
            if (outer)
                entities.push(outer);
        }
        if (hasInnerArcs) {
            for (var i = 1; i < Math.floor(points.length / 2); i++) {
                ...
                if (!left || !right || left === right)
                    continue;
                var inner = this.drawCapArcBetweenElements(renderer, side, left, right, capDirection, traits, originalColor, originalLineType, mlineStyle);
                if (inner)
                    entities.push(inner);
            }
        }
     ...
     */
    AcDbMLine.prototype.drawCapArcBetweenElements = function (renderer, side, startPointPair, endPointPair, capDirection, traits, originalColor, originalLineType, mlineStyle) {
        ...
        if (!arc)
            return undefined;
        this.applyElementDrawTraits(traits, originalColor, originalLineType, mlineStyle, side === 'end' ? endPointPair.elementIndex : startPointPair.elementIndex);
     ...
     * @param elementIndex Style element index.
     */
    AcDbMLine.prototype.applyElementDrawTraits = function (traits, originalColor, originalLineType, mlineStyle, elementIndex) {
        traits.color = originalColor;
        traits.lineType = originalLineType;
        this.applyStyleElementTraits(mlineStyle, elementIndex, traits);
    };
