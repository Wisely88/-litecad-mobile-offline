...
var AcDbPolyFaceMesh = /** @class */ (function (_super) {
    ...
    Object.defineProperty(AcDbPolyFaceMesh.prototype, "area", {
        /** @inheritdoc */
        get: function () {
            return 0;
        },
    ...
    /** @inheritdoc */
    AcDbPolyFaceMesh.prototype.subGetIntersectCurves = function () {
        var _this = this;
        var primitives = [];
        this._faces.forEach(function (face) {
            ...
            if (faceVertices.length >= 2) {
                primitives.push.apply(primitives, __spreadArray([], __read(acdbIntersectPrimitivesFromPointPath(faceVertices, true)), false));
            }
        });
        return primitives;
    };
    ...
    /** @inheritdoc */
    AcDbPolyFaceMesh.prototype.subMoveGripPointsAt = function (indices, offset) {
        var _this = this;
     ...
     */
    AcDbPolyFaceMesh.prototype.transformBy = function (matrix) {
        this._vertices.forEach(function (vertex) {
     ...
     * in storage order.
     */
    AcDbPolyFaceMesh.prototype.getOffsetCurves = function (offsetDist) {
     ...
     */
    AcDbPolyFaceMesh.prototype.getOffsetSideAtPoint = function (point) {
        var path = this.collectLargestFaceBoundary2d();
        if (path.length < 2)
            return 1;
     ...
     */
    AcDbPolyFaceMesh.prototype.createOffsetCurve = function (offsetDist) {
        return acdbOffsetVertexPathAsPolyline(this.collectLargestFaceBoundary2d(), true, offsetDist);
    };
    /**
     ...
     */
    AcDbPolyFaceMesh.prototype.collectLargestFaceBoundary2d = function () {
        var e_1, _a;
        var _this = this;
        var bestIndices = [];
        try {
            for (var _b = __values(this._faces), _c = _b.next(); !_c.done; _c = _b.next()) {
                var face = _c.value;
                var indices = face.vertexIndices.filter(function (index) { return index >= 0; });
        ...
        if (bestIndices.length < 2) {
            return this._vertices.map(function (vertex) { return new AcGePoint2d(vertex.position.x, vertex.position.y); });
        }
        return bestIndices.map(function (index) {
            var position = _this.getVertexAt(index).position;
