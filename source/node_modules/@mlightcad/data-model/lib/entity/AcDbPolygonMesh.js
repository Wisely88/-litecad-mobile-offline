var __extends = (this && this.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (Object.prototype.hasOwnProperty.call(b, p)) d[p] = b[p]; };
        return extendStatics(d, b);
    };
    return function (d, b) {
        if (typeof b !== "function" && b !== null)
            throw new TypeError("Class extends value " + String(b) + " is not a constructor or null");
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
var __read = (this && this.__read) || function (o, n) {
    var m = typeof Symbol === "function" && o[Symbol.iterator];
    if (!m) return o;
    var i = m.call(o), r, ar = [], e;
    try {
        while ((n === void 0 || n-- > 0) && !(r = i.next()).done) ar.push(r.value);
    }
    catch (error) { e = { error: error }; }
    finally {
        try {
            if (r && !r.done && (m = i["return"])) m.call(i);
        }
        finally { if (e) throw e.error; }
    }
    return ar;
};
var __spreadArray = (this && this.__spreadArray) || function (to, from, pack) {
    if (pack || arguments.length === 2) for (var i = 0, l = from.length, ar; i < l; i++) {
        if (ar || !(i in from)) {
            if (!ar) ar = Array.prototype.slice.call(from, 0, i);
            ar[i] = from[i];
        }
    }
    return to.concat(ar || Array.prototype.slice.call(from));
};
import { AcGeBox3d, AcGePoint2d, AcGePoint3d } from '@mlightcad/geometry-engine';
import { AcDbOsnapMode } from '../misc/AcDbOsnapMode';
import { AcDbCurve } from './AcDbCurve';
import { acdbForEachGripIndex } from './AcDbGripHelpers';
import { acdbIntersectPrimitivesFromPointPath } from './AcDbIntersectHelpers';
import { acdbOffsetVertexPathAsPolyline, AcDbPolyline } from './AcDbPolyline';
/**
 * Represents a polygon mesh vertex in AutoCAD.
 */
var AcDbPolygonMeshVertex = /** @class */ (function () {
    /**
     * Creates a new polygon mesh vertex.
     * @param position The 3D position of the vertex
     */
    function AcDbPolygonMeshVertex(position) {
        this.position = new AcGePoint3d(position.x, position.y, position.z || 0);
    }
    return AcDbPolygonMeshVertex;
}());
export { AcDbPolygonMeshVertex };
/**
 * Represents a polygon mesh entity in AutoCAD.
 */
var AcDbPolygonMesh = /** @class */ (function (_super) {
    __extends(AcDbPolygonMesh, _super);
    /**
     * Creates a new polygon mesh entity.
     * @param mCount The number of vertices in the M direction
     * @param nCount The number of vertices in the N direction
     * @param vertices The vertices of the mesh
     * @param closedM Whether the mesh is closed in the M direction
     * @param closedN Whether the mesh is closed in the N direction
     */
    function AcDbPolygonMesh(mCount, nCount, vertices, closedM, closedN) {
        if (closedM === void 0) { closedM = false; }
        if (closedN === void 0) { closedN = false; }
        var _this = _super.call(this) || this;
        _this._mCount = mCount;
        _this._nCount = nCount;
        _this._closedM = closedM;
        _this._closedN = closedN;
        _this._vertices = vertices.map(function (v) { return new AcDbPolygonMeshVertex(v); });
        return _this;
    }
    Object.defineProperty(AcDbPolygonMesh.prototype, "dxfTypeName", {
        get: function () {
            return 'POLYLINE';
        },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(AcDbPolygonMesh.prototype, "mCount", {
        /**
         * Gets the number of vertices in the M direction.
         */
        get: function () {
            return this._mCount;
        },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(AcDbPolygonMesh.prototype, "nCount", {
        /**
         * Gets the number of vertices in the N direction.
         */
        get: function () {
            return this._nCount;
        },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(AcDbPolygonMesh.prototype, "closedM", {
        /**
         * Gets whether the mesh is closed in the M direction.
         */
        get: function () {
            return this._closedM;
        },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(AcDbPolygonMesh.prototype, "closedN", {
        /**
         * Gets whether the mesh is closed in the N direction.
         */
        get: function () {
            return this._closedN;
        },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(AcDbPolygonMesh.prototype, "closed", {
        /**
         * Gets whether this mesh is closed.
         */
        get: function () {
            return this._closedM;
        },
        /**
         * Sets whether this mesh is closed.
         */
        set: function (value) {
            this._closedM = value;
        },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(AcDbPolygonMesh.prototype, "area", {
        /** @inheritdoc */
        get: function () {
            return 0;
        },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(AcDbPolygonMesh.prototype, "numberOfVertices", {
        /**
         * Gets the number of vertices in the mesh.
         */
        get: function () {
            return this._vertices.length;
        },
        enumerable: false,
        configurable: true
    });
    /**
     * Gets the vertex at the specified index.
     * @param index The index of the vertex
     */
    AcDbPolygonMesh.prototype.getVertexAt = function (index) {
        if (index < 0 || index >= this._vertices.length) {
            throw new Error('Vertex index out of bounds');
        }
        return this._vertices[index];
    };
    /**
     * Gets the vertex at the specified M and N coordinates.
     * @param m The M coordinate
     * @param n The N coordinate
     */
    AcDbPolygonMesh.prototype.getVertexAtMN = function (m, n) {
        var index = m * this._nCount + n;
        return this.getVertexAt(index);
    };
    Object.defineProperty(AcDbPolygonMesh.prototype, "geometricExtents", {
        /**
         * Gets the geometric extents (bounding box) of this mesh.
         */
        get: function () {
            if (this._vertices.length === 0) {
                return new AcGeBox3d(new AcGePoint3d(0, 0, 0), new AcGePoint3d(0, 0, 0));
            }
            var minX = Number.MAX_VALUE;
            var minY = Number.MAX_VALUE;
            var minZ = Number.MAX_VALUE;
            var maxX = -Number.MAX_VALUE;
            var maxY = -Number.MAX_VALUE;
            var maxZ = -Number.MAX_VALUE;
            this._vertices.forEach(function (vertex) {
                minX = Math.min(minX, vertex.position.x);
                minY = Math.min(minY, vertex.position.y);
                minZ = Math.min(minZ, vertex.position.z);
                maxX = Math.max(maxX, vertex.position.x);
                maxY = Math.max(maxY, vertex.position.y);
                maxZ = Math.max(maxZ, vertex.position.z);
            });
            return new AcGeBox3d(new AcGePoint3d(minX, minY, minZ), new AcGePoint3d(maxX, maxY, maxZ));
        },
        enumerable: false,
        configurable: true
    });
    /** @inheritdoc */
    AcDbPolygonMesh.prototype.subGetIntersectCurves = function () {
        var primitives = [];
        for (var m = 0; m < this._mCount; m++) {
            for (var n = 0; n < this._nCount; n++) {
                var current = this.getVertexAtMN(m, n);
                var nextN = n + 1;
                if (nextN >= this._nCount) {
                    if (this._closedN)
                        nextN = 0;
                    else
                        continue;
                }
                primitives.push.apply(primitives, __spreadArray([], __read(acdbIntersectPrimitivesFromPointPath([current.position, this.getVertexAtMN(m, nextN).position], false, false)), false));
            }
        }
        for (var n = 0; n < this._nCount; n++) {
            for (var m = 0; m < this._mCount; m++) {
                var current = this.getVertexAtMN(m, n);
                var nextM = m + 1;
                if (nextM >= this._mCount) {
                    if (this._closedM)
                        nextM = 0;
                    else
                        continue;
                }
                primitives.push.apply(primitives, __spreadArray([], __read(acdbIntersectPrimitivesFromPointPath([current.position, this.getVertexAtMN(nextM, n).position], false, false)), false));
            }
        }
        return primitives;
    };
    /**
     * Gets the grip points for this mesh.
     */
    AcDbPolygonMesh.prototype.subGetGripPoints = function () {
        var gripPoints = new Array();
        this._vertices.forEach(function (vertex) {
            gripPoints.push(vertex.position);
        });
        return gripPoints;
    };
    /** @inheritdoc */
    AcDbPolygonMesh.prototype.subMoveGripPointsAt = function (indices, offset) {
        var _this = this;
        acdbForEachGripIndex(indices, function (index) {
            var vertex = _this._vertices[index];
            if (vertex) {
                vertex.position.add(offset);
            }
        });
        return this;
    };
    /**
     * Gets the object snap points for this mesh.
     */
    AcDbPolygonMesh.prototype.subGetOsnapPoints = function (osnapMode, _pickPoint, _lastPoint, snapPoints) {
        switch (osnapMode) {
            case AcDbOsnapMode.EndPoint:
                this._vertices.forEach(function (vertex) {
                    snapPoints.push(vertex.position);
                });
                break;
            default:
                break;
        }
    };
    /**
     * Transforms this polygon mesh by the specified matrix.
     */
    AcDbPolygonMesh.prototype.transformBy = function (matrix) {
        this._vertices.forEach(function (vertex) {
            vertex.position.applyMatrix4(matrix);
        });
        return this;
    };
    Object.defineProperty(AcDbPolygonMesh.prototype, "properties", {
        /**
         * Returns the full property definition for this mesh entity.
         */
        get: function () {
            var _this = this;
            return {
                type: this.type,
                groups: [
                    this.getGeneralProperties(),
                    {
                        groupName: 'geometry',
                        properties: [
                            {
                                name: 'mCount',
                                type: 'float',
                                editable: false,
                                accessor: {
                                    get: function () { return _this._mCount; }
                                }
                            },
                            {
                                name: 'nCount',
                                type: 'float',
                                editable: false,
                                accessor: {
                                    get: function () { return _this._nCount; }
                                }
                            },
                            {
                                name: 'vertices',
                                type: 'array',
                                editable: false,
                                itemSchema: {
                                    properties: [
                                        {
                                            name: 'x',
                                            type: 'float',
                                            editable: true
                                        },
                                        {
                                            name: 'y',
                                            type: 'float',
                                            editable: true
                                        },
                                        {
                                            name: 'z',
                                            type: 'float',
                                            editable: true
                                        }
                                    ]
                                },
                                accessor: {
                                    get: function () {
                                        return _this._vertices.map(function (v) { return ({
                                            x: v.position.x,
                                            y: v.position.y,
                                            z: v.position.z
                                        }); });
                                    }
                                }
                            }
                        ]
                    },
                    {
                        groupName: 'others',
                        properties: [
                            {
                                name: 'closedM',
                                type: 'boolean',
                                editable: true,
                                accessor: {
                                    get: function () { return _this._closedM; },
                                    set: function (v) {
                                        _this._closedM = v;
                                    }
                                }
                            },
                            {
                                name: 'closedN',
                                type: 'boolean',
                                editable: true,
                                accessor: {
                                    get: function () { return _this._closedN; },
                                    set: function (v) {
                                        _this._closedN = v;
                                    }
                                }
                            }
                        ]
                    }
                ]
            };
        },
        enumerable: false,
        configurable: true
    });
    /**
     * Draws this mesh using the specified renderer.
     */
    AcDbPolygonMesh.prototype.subWorldDraw = function (renderer) {
        var lines = [];
        // Draw M-direction lines
        for (var m = 0; m < this._mCount; m++) {
            for (var n = 0; n < this._nCount; n++) {
                var current = this.getVertexAtMN(m, n);
                var nextN = n + 1;
                if (nextN >= this._nCount) {
                    if (this._closedN) {
                        nextN = 0;
                    }
                    else {
                        continue;
                    }
                }
                var next = this.getVertexAtMN(m, nextN);
                lines.push(current.position);
                lines.push(next.position);
            }
        }
        // Draw N-direction lines
        for (var n = 0; n < this._nCount; n++) {
            for (var m = 0; m < this._mCount; m++) {
                var current = this.getVertexAtMN(m, n);
                var nextM = m + 1;
                if (nextM >= this._mCount) {
                    if (this._closedM) {
                        nextM = 0;
                    }
                    else {
                        continue;
                    }
                }
                var next = this.getVertexAtMN(nextM, n);
                lines.push(current.position);
                lines.push(next.position);
            }
        }
        return renderer.lines(lines);
    };
    /**
     * Writes DXF fields for this object.
     */
    AcDbPolygonMesh.prototype.dxfOutFields = function (filer) {
        _super.prototype.dxfOutFields.call(this, filer);
        filer.writeSubclassMarker('AcDbPolygonMesh');
        var flag = 16; // Polygon mesh flag
        if (this._closedM)
            flag |= 1;
        if (this._closedN)
            flag |= 32;
        filer.writeInt16(66, 1); // Has vertices
        filer.writeInt16(70, flag);
        filer.writeInt32(71, this._mCount);
        filer.writeInt32(72, this._nCount);
        return this;
    };
    /**
     * Writes this object to the DXF output.
     */
    AcDbPolygonMesh.prototype.dxfOut = function (filer, allXdata) {
        if (allXdata === void 0) { allXdata = false; }
        _super.prototype.dxfOut.call(this, filer, allXdata);
        for (var i = 0; i < this.numberOfVertices; i++) {
            var vertex = this.getVertexAt(i);
            filer.writeStart('VERTEX');
            filer.writeHandle(5, this.database.generateHandle());
            filer.writeObjectId(330, this.objectId);
            filer.writeSubclassMarker('AcDbEntity');
            filer.writeSubclassMarker('AcDbVertex');
            filer.writeSubclassMarker('AcDbPolygonMeshVertex');
            filer.writePoint3d(10, vertex.position);
            filer.writeInt16(70, 16); // Polygon mesh vertex flag
        }
        filer.writeStart('SEQEND');
        filer.writeHandle(5, this.database.generateHandle());
        filer.writeObjectId(330, this.objectId);
        filer.writeSubclassMarker('AcDbEntity');
        return this;
    };
    /**
     * {@inheritDoc AcDbCurve.getOffsetCurves}
     *
     * Offsets the mesh perimeter in XY: a single row/column for degenerate meshes, or a
     * counter-clockwise walk around the M×N grid when both dimensions exceed one.
     */
    AcDbPolygonMesh.prototype.getOffsetCurves = function (offsetDist) {
        var curve = this.createOffsetCurve(offsetDist);
        return curve ? [curve] : [];
    };
    /**
     * {@inheritDoc AcDbCurve.getOffsetSideAtPoint}
     *
     * Uses the same perimeter path and closed flag as {@link getOffsetCurves}.
     */
    AcDbPolygonMesh.prototype.getOffsetSideAtPoint = function (point) {
        var path = this.collectBoundary2d();
        if (path.length < 2)
            return 1;
        return AcDbPolyline.from2dPoints(path, this.isBoundaryClosed()).getOffsetSideAtPoint(point);
    };
    /**
     * @param offsetDist - Signed offset distance in drawing units
     * @returns Offset polyline along the mesh boundary, or `null` on failure
     */
    AcDbPolygonMesh.prototype.createOffsetCurve = function (offsetDist) {
        return acdbOffsetVertexPathAsPolyline(this.collectBoundary2d(), this.isBoundaryClosed(), offsetDist);
    };
    /**
     * Whether the collected perimeter should be offset as a closed polygon.
     *
     * Requires both M and N closure flags and at least two vertices along each direction.
     */
    AcDbPolygonMesh.prototype.isBoundaryClosed = function () {
        return (this._closedM && this._closedN && this._mCount > 1 && this._nCount > 1);
    };
    /**
     * Builds the XY perimeter of the polygon mesh for offset operations.
     *
     * For `M === 1` or `N === 1`, returns the sole row or column. Otherwise traces the
     * outer ring: first column up, last row across, last column down, first row back.
     *
     * @returns Boundary vertices in traversal order
     */
    AcDbPolygonMesh.prototype.collectBoundary2d = function () {
        var _this = this;
        var m = this._mCount;
        var n = this._nCount;
        if (m < 1 || n < 1)
            return [];
        var points = [];
        var push = function (mi, ni) {
            var position = _this.getVertexAtMN(mi, ni).position;
            points.push(new AcGePoint2d(position.x, position.y));
        };
        if (m === 1) {
            for (var j = 0; j < n; j++)
                push(0, j);
            return points;
        }
        if (n === 1) {
            for (var i = 0; i < m; i++)
                push(i, 0);
            return points;
        }
        for (var j = 0; j < n; j++)
            push(0, j);
        for (var i = 1; i < m; i++)
            push(i, n - 1);
        for (var j = n - 2; j >= 0; j--)
            push(m - 1, j);
        for (var i = m - 2; i >= 1; i--)
            push(i, 0);
        return points;
    };
    /** The entity type name */
    AcDbPolygonMesh.typeName = 'PolygonMesh';
    return AcDbPolygonMesh;
}(AcDbCurve));
export { AcDbPolygonMesh };
//# sourceMappingURL=AcDbPolygonMesh.js.map