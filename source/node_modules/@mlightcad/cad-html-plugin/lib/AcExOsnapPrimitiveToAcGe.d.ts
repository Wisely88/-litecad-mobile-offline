/**
 * Converts exported {@link AcExOsnapPrimitive} records into `AcGe*` curves
 * for object snap evaluation in the offline HTML viewer.
 *
 * @packageDocumentation
 */
import { AcGeCircArc2d, AcGeEllipseArc2d, AcGeLine2d, AcGeNurbsCurve, AcGePoint2d } from '@mlightcad/data-model';
import type { AcExOsnapArcPrimitive, AcExOsnapCirclePrimitive, AcExOsnapEllipsePrimitive, AcExOsnapPrimitive, AcExOsnapSplinePrimitive } from './AcExOsnapPrimitiveTypes';
/** Discriminated result of {@link primitiveToAcGeCurve}. */
export type AcExOsnapAcGeCurve = {
    kind: 'line';
    curve: AcGeLine2d;
} | {
    kind: 'circArc';
    curve: AcGeCircArc2d;
} | {
    kind: 'ellipse';
    curve: AcGeEllipseArc2d;
} | {
    kind: 'spline';
    curve: AcGeNurbsCurve;
} | {
    kind: 'point';
    point: AcGePoint2d;
};
/** Positive CCW sweep from start to end angle (radians). */
export declare function normalizeArcDelta(startAngle: number, endAngle: number): number;
/** WCS point on a circular arc from center/radius and OCS-style angle. */
export declare function arcWcsPoint(cx: number, cy: number, r: number, angle: number, normalSign: 1 | -1): AcGePoint2d;
/**
 * Inverse of {@link arcWcsPoint}: OCS parameter angle of a WCS point.
 *
 * Used when exporting WCS {@link AcGeCircArc2d} bulge geometry (polylines,
 * hatch loops). Clockwise `AcGeCircArc2d` public angles are Y-mirrored, which
 * is not the AutoCAD −Z / X-mirror convention {@link arcToAcGe} expects.
 */
export declare function wcsPointToOcsArcAngle(cx: number, cy: number, x: number, y: number, normalSign: 1 | -1): number;
/**
 * Builds an open circular arc in WCS from exported center/angle data.
 *
 * AutoCAD stores arc angles in OCS; a `-Z` extrusion mirrors X when mapping to
 * WCS. {@link AcGeCircArc2d}'s `clockwise` flag alone does not reproduce that,
 * so the arc is reconstructed from WCS endpoints and bulge.
 */
export declare function arcToAcGe(prim: AcExOsnapArcPrimitive): AcGeCircArc2d;
/**
 * Positive parameter sweep for an elliptical arc (radians).
 *
 * @param prim - Exported ellipse primitive.
 */
export declare function ellipseArcDelta(prim: AcExOsnapEllipsePrimitive): number;
/**
 * Evaluates a WCS point on an elliptical arc at normalized parameter `u` ∈ [0, 1].
 *
 * Matches AutoCAD / `AcDbEllipse` OCS→WCS mapping (including `-Z` extrusion).
 *
 * @param prim - Exported ellipse primitive.
 * @param u - Normalized parameter along the arc (`0` = start, `1` = end).
 */
export declare function ellipsePointAtNormalized(prim: AcExOsnapEllipsePrimitive, u: number): AcGePoint2d;
/**
 * Builds an `AcGeCircArc2d` from a circle or arc primitive.
 *
 * @param prim - Circle or arc in WCS.
 */
export declare function circleOrArcToAcGe(prim: AcExOsnapCirclePrimitive | AcExOsnapArcPrimitive): AcGeCircArc2d;
/**
 * Builds an `AcGeEllipseArc2d` from an ellipse primitive.
 *
 * @param prim - Ellipse or elliptical arc in WCS.
 */
export declare function ellipseToAcGe(prim: AcExOsnapEllipsePrimitive): AcGeEllipseArc2d;
/**
 * Builds an `AcGeNurbsCurve` from a spline primitive.
 *
 * @param prim - Serialized NURBS data in WCS.
 */
export declare function splineToAcGe(prim: AcExOsnapSplinePrimitive): AcGeNurbsCurve;
/**
 * Maps one snapshot primitive to the corresponding `AcGe` curve or point.
 *
 * @param prim - Exported primitive from {@link AcExOsnapCatalog}.
 * @returns Wrapped geometry for snap evaluation.
 */
export declare function primitiveToAcGeCurve(prim: AcExOsnapPrimitive): AcExOsnapAcGeCurve;
//# sourceMappingURL=AcExOsnapPrimitiveToAcGe.d.ts.map