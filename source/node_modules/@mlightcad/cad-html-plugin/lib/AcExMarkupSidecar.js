    ...
    return {
        ...
        style: {
            ...
            fontSize: typeof raw.style.fontSize === 'number' && raw.style.fontSize > 0
                ? raw.style.fontSize
                : undefined,
            textHeightMode: raw.style.textHeightMode === 'custom' ||
 ...
 */
export function parseAcExMarkupSidecar(text) {
    var e_1, _a;
    var parsed;
    try {
...
function normalizeStyleForWrite(style) {
    ...
    return __assign(__assign({}, rest), { lineWeight: ACEX_MARKUP_LINE_WEIGHT });
}
/** Serialize a sidecar file to pretty-printed JSON. */
export function stringifyAcExMarkupSidecar(file) {
 ...
 */
export function acexMarkupSidecarFileName(drawingName) {
    if (!drawingName)
        return 'drawing.markup.json';
    var base = drawingName.replace(/\.(dwg|dxf|html)$/i, '');
    return "".concat(base, ".markup.json");
}
//# sourceMappingURL=AcExMarkupSidecar.js.map