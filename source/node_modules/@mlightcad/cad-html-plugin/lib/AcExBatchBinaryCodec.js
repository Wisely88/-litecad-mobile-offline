...
export var F_LINE_INDICES = 1;
export var F_LINE_PATTERN = 2;
export var F_LINE_DISTANCES = 4;
export var F_LINE_WIDTH = 8;
export var F_LINE_RENDER_ORDER = 16;
/** Text/point glyph strokes — display only; skip hybrid OSNAP. */
export var F_LINE_EXCLUDE_OSNAP = 32;
/** Optional mesh-batch feature flags (append-only; never renumber). */
export var F_MESH_INDICES = 1;
export var F_MESH_HATCH = 2;
export var F_MESH_GRADIENT_FILL = 4;
export var F_MESH_GRADIENT_POS = 8;
export var F_MESH_SIDE = 16;
export var F_MESH_POINTS = 32;
export var F_MESH_RENDER_ORDER = 64;
/** v4: uvs + embedded raster texture (IMAGE / OLE). */
export var F_MESH_TEXTURE = 128;
/**
 ...
 */
export function estimateLineBatchBytes(batch) {
    var size = 256 + batch.positions.byteLength;
    if (batch.indices)
        size += batch.indices.byteLength;
    if (batch.lineDistances)
        size += batch.lineDistances.byteLength;
    if (batch.linePattern)
        size += 64 + batch.linePattern.pattern.length * 8;
    return size;
}
/**
 * Rough uncompressed payload size for chunk-size budgeting.
 */
export function estimateMeshBatchBytes(batch) {
    var size = 256 + batch.positions.byteLength;
    if (batch.indices)
        size += batch.indices.byteLength;
    if (batch.gradientPositions)
        size += batch.gradientPositions.byteLength;
    if (batch.uvs)
        size += batch.uvs.byteLength;
    if (batch.texture)
        size += batch.texture.bytes.byteLength + 64;
...
export function writeLineBatch(writer, batch) {
...
export function readLineBatch(reader) {
...
export function writeMeshBatch(writer, batch) {
    writer.writeString(batch.layer);
    writer.writeU32(batch.color >>> 0);
    writer.writeF64(batch.offset[0]);
    writer.writeF64(batch.offset[1]);
...
export function readMeshBatch(reader) {
    var layer = reader.readString();
    var color = reader.readU32();
    var offset = [
        reader.readF64(),
    ...
    if (flags & F_MESH_TEXTURE) {
        ...
        var mimeType = reader.readString() || 'image/png';
        var byteLength = reader.readU32();
        batch.texture = {
            mimeType: mimeType,
...
//# sourceMappingURL=AcExBatchBinaryCodec.js.map