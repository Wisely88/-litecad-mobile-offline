 ...
 * @internal
 */
export function copyFloat32Range(array, start, count) {
    if (count <= 0) {
 ...
 * @internal
 */
export function copyUint32Range(array, start, count) {
    if (count <= 0) {
 ...
 */
export function compactIndexedSlice(positions, indices) {
    if (indices.length === 0) {
    ...
    var vertexFloatCount = (maxIndex + 1) * 3;
 ...
 */
export function toWcsCoord(local, origin) {
    return local + origin;
}
var _worldOffset = /*@__PURE__*/ { x: 0, y: 0, z: 0 };
/**
 ...
 */
export function readBatchWorldOffset(object) {
    object.updateMatrixWorld(true);
    var position = object.matrixWorld.elements;
 ...
 */
export function bakePlainDrawableSlice(slice, matrix) {
    var elements = matrix.elements;
    var source = slice.positions;
    if (source.length === 0) {
        return { positions: new Float32Array(0), indices: slice.indices };
    }
    var transformed = new Float32Array(source.length);
 ...
 */
export function rebasePlainDrawableSliceAroundCentroid(slice) {
    var source = slice.positions;
    if (source.length < 3) {
        return { slice: slice, offset: [0, 0, 0] };
    }
    var minX = Infinity;
    var minY = Infinity;
    var minZ = Infinity;
    ...
    var rebased = new Float32Array(source.length);
    for (var i = 0; i < source.length; i += 3) {
 ...
 */
export function exportPlainDrawableSlice(object, slice, _options) {
    if (_options === void 0) { _options = {}; }
    object.updateMatrixWorld(true);
    var worldSlice = bakePlainDrawableSlice(slice, object.matrixWorld);
...
//# sourceMappingURL=AcExBatchBuffers.js.map