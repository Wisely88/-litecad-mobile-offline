/**
 * Object snap candidate collection for exported analytic primitives.
 *
 * Thin orchestration layer: converts {@link AcExOsnapPrimitive} via
 * {@link primitiveToAcGeCurve}, then calls `AcGeLine2d`, `AcGeCircArc2d`,
 * `AcGeEllipseArc2d`, and `AcGeNurbsCurve` from `@mlightcad/data-model`
 * (geometry-engine) for all snap math.
 *
 * @packageDocumentation
 */
import { AcGeCircArc2d, type AcGePoint2dLike } from '@mlightcad/data-model';
import { type AcExOsnapAcGeCurve } from './AcExOsnapPrimitiveToAcGe';
import type { AcExOsnapMode, AcExOsnapPrimitive } from './AcExOsnapPrimitiveTypes';
/**
 * Squared Euclidean distance between two XY points.
 *
 * @param ax - First point X.
 * @param ay - First point Y.
 * @param bx - Second point X.
 * @param by - Second point Y.
 */
export declare function distSq(ax: number, ay: number, bx: number, by: number): number;
/**
 * How much `mouse - nearest` points into this circular arc.
 *
 * When two polyline bulge segments share a vertex, nearest-point distances
 * are equal and a later-wins compare would always lock the second arc.
 */
export declare function inwardArcAlignment(curve: AcGeCircArc2d, nearest: AcGePoint2dLike, mouse: AcGePoint2dLike): number;
/** True when `distSqValue`/`align` should replace the current lock winner. */
export declare function isBetterArcLock(distSqValue: number, align: number, bestDistSq: number, bestAlign: number): boolean;
/** One snap candidate before distance filtering in {@link AcExOsnapIndex}. */
export interface AcExOsnapCandidate {
    x: number;
    y: number;
    mode: AcExOsnapMode;
}
/**
 * Collects discrete (non-nearest) snap points for one primitive during a snap query.
 *
 * @param prim - Exported primitive in WCS.
 * @param modes - Enabled OSNAP modes.
 * @param geo - Optional pre-built curve from {@link primitiveToAcGeCurve}.
 */
export declare function collectPrimitiveDiscreteSnapCandidates(prim: AcExOsnapPrimitive, modes: Set<AcExOsnapMode>, geo?: AcExOsnapAcGeCurve): AcExOsnapCandidate[];
/**
 * Nearest snap on one primitive; uses a pre-built curve when provided.
 */
export declare function collectPrimitiveNearestSnapCandidate(prim: AcExOsnapPrimitive, px: number, py: number, geo?: AcExOsnapAcGeCurve): AcExOsnapCandidate | undefined;
/**
 * Collects snap candidates for one primitive using `AcGe*` geometry.
 *
 * @param prim - Exported primitive in WCS.
 * @param px - Pick X in WCS.
 * @param py - Pick Y in WCS.
 * @param modes - Enabled OSNAP modes.
 */
export declare function collectPrimitiveSnapCandidates(prim: AcExOsnapPrimitive, px: number, py: number, modes: Set<AcExOsnapMode>): AcExOsnapCandidate[];
//# sourceMappingURL=AcExOsnapGeometry.d.ts.map