...
import { ICON_ANNOTATION, ICON_CHEVRON_DOWN, ICON_CHEVRON_UP, ICON_CLEAR_MARKUPS, ICON_CLEAR_MEASUREMENTS, ICON_COLOR, ICON_LANGUAGE, ICON_LAYER, ICON_LAYER_OFF, ICON_LAYER_ON, ICON_LAYOUT, ICON_MARKUP_ARROW, ICON_MARKUP_CALLOUT, ICON_MARKUP_CIRCLE, ICON_MARKUP_CLOUD, ICON_MARKUP_EXPORT, ICON_MARKUP_HIDE, ICON_MARKUP_IMPORT, ICON_MARKUP_PANEL, ICON_MARKUP_RECT, ICON_MARKUP_SHOW, ICON_MARKUP_STAMP, ICON_MARKUP_TEXT, ICON_MEASURE, ICON_MEASURE_ANGLE, ICON_MEASURE_ARC, ICON_MEASURE_AREA, ICON_MEASURE_CONTINUOUS, ICON_MEASURE_COORDINATE, ICON_MEASURE_DISTANCE, ICON_MEASUREMENT_PANEL, ICON_ORTHO_MODE, ICON_OSNAP, ICON_PAN, ICON_POLAR_TRACKING, ICON_SELECT, ICON_SETTINGS, ICON_SIMULATED_MOUSE, ICON_SWITCH_BG, ICON_THEME_DARK, ICON_THEME_LIGHT, ICON_ZOOM_BOX, ICON_ZOOM_EXTENT, ICON_ZOOM_ORIGINAL, ICON_ZOOM_WINDOW } from '@mlightcad/cad-simple-viewer/icons';
/**
 * Inline SVG markup keyed by toolbar / layer UI usage.
 * Each value is a complete `<svg>…</svg>` string using `currentColor`.
 */
export var AcExHtmlIcons = {
 ...
 */
export function acexToolbarButton(icon, title, attrs) {
    var attrStr = Object.entries(attrs)
        ...
        .join(' ');
    var i18nKey = attrs['data-i18n-key'];
    var label = i18nKey
        ? "<span class=\"mlcad-tool-btn-label\" data-i18n-key=\"".concat(escapeAttr(i18nKey), "\" data-i18n-text>").concat(escapeAttr(title), "</span>")
        : '';
    return "<button type=\"button\" class=\"mlcad-tool-btn\" title=\"".concat(escapeAttr(title), "\" aria-label=\"").concat(escapeAttr(title), "\" ").concat(attrStr, "><span class=\"mlcad-tool-btn-icon\" aria-hidden=\"true\">").concat(icon, "</span>").concat(label, "</button>");
}
/**
 ...
 * @param attrs - Additional attributes (e.g. `data-action`, `data-i18n-key`).
 */
export function acexDropdownItem(icon, label, attrs) {
    var attrStr = Object.entries(attrs)
        .map(function (_a) {
        var _b = __read(_a, 2), key = _b[0], value = _b[1];
        return "".concat(key, "=\"").concat(escapeAttr(value), "\"");
        ...
        .join(' ');
    var i18nKey = attrs['data-i18n-key'];
    var labelAttrs = i18nKey
        ? " data-i18n-key=\"".concat(escapeAttr(i18nKey), "\" data-i18n-text")
        : '';
    return "<button type=\"button\" class=\"mlcad-dropdown-item\" role=\"menuitem\" title=\"".concat(escapeAttr(label), "\" ").concat(attrStr, "><span class=\"mlcad-dropdown-icon\" aria-hidden=\"true\">").concat(icon, "</span><span class=\"mlcad-dropdown-label\"").concat(labelAttrs, ">").concat(escapeAttr(label), "</span></button>");
}
...
//# sourceMappingURL=AcExHtmlIcons.js.map