 ...
 */
import { AcGeCircArc2d, AcGeEllipseArc2d, AcGeLine2d, AcGeNurbsCurve, AcGePoint2d, TAU } from '@mlightcad/data-model';
/** Positive CCW sweep from start to end angle (radians). */
export function normalizeArcDelta(startAngle, endAngle) {
...
export function arcWcsPoint(cx, cy, r, angle, normalSign) {
    var sx = normalSign === -1 ? -1 : 1;
    return new AcGePoint2d(cx + sx * r * Math.cos(angle), cy + r * Math.sin(angle));
}
/**
 ...
 */
export function wcsPointToOcsArcAngle(cx, cy, x, y, normalSign) {
 ...
 */
export function arcToAcGe(prim) {
    var start = arcWcsPoint(prim.cx, prim.cy, prim.r, prim.startAngle, prim.normalSign);
    var end = arcWcsPoint(prim.cx, prim.cy, prim.r, prim.endAngle, prim.normalSign);
    var delta = normalizeArcDelta(prim.startAngle, prim.endAngle);
    var bulge = prim.normalSign * Math.tan(delta / 4);
 ...
 */
export function ellipseArcDelta(prim) {
 ...
 */
export function ellipsePointAtNormalized(prim, u) {
    var _a;
    var delta = ellipseArcDelta(prim);
    var t = prim.startAngle + u * delta;
    var normalSign = (_a = prim.normalSign) !== null && _a !== void 0 ? _a : 1;
    var lx = prim.majorR * Math.cos(t);
    var ly = normalSign * prim.minorR * Math.sin(t);
    var c = prim.majorX;
    var s = prim.majorY;
 ...
 */
export function circleOrArcToAcGe(prim) {
    if (prim.kind === 'circle') {
        return new AcGeCircArc2d({ x: prim.cx, y: prim.cy }, prim.r, 0, TAU, prim.normalSign === -1);
    }
 ...
 */
export function ellipseToAcGe(prim) {
    var rotation = Math.atan2(prim.majorY, prim.majorX);
    return new AcGeEllipseArc2d({ x: prim.cx, y: prim.cy, z: 0 }, prim.majorR, prim.minorR, prim.startAngle, prim.endAngle, prim.closed, rotation);
}
/**
 ...
 */
export function splineToAcGe(prim) {
    var controlPoints = [];
    ...
    return new AcGeNurbsCurve(prim.degree, prim.knots, controlPoints, prim.weights.length > 0 ? prim.weights : undefined);
}
/**
 ...
 */
export function primitiveToAcGeCurve(prim) {
    switch (prim.kind) {
        case 'line':
            return {
                kind: 'line',
                curve: new AcGeLine2d({ x: prim.x0, y: prim.y0 }, { x: prim.x1, y: prim.y1 })
            };
        case 'circle':
        case 'arc':
...
//# sourceMappingURL=AcExOsnapPrimitiveToAcGe.js.map