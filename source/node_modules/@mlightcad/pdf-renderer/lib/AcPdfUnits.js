...
export var PDF_POINTS_PER_MM = 72 / 25.4;
/** Adobe's practical MediaBox limit in user units. */
export var PDF_MAX_PAGE_SIZE = 14400;
export function mmPerDrawingUnit(insunits) {
    var _a;
    if (!Number.isFinite(insunits)) {
        return 1;
    }
    return (_a = ACAD_UNIT_TO_MM[Math.trunc(insunits)]) !== null && _a !== void 0 ? _a : 1;
}
/**
 * Scale that maps one drawing unit into PDF points, honouring INSUNITS.
 */
export function drawingUnitsToPdfPoints(insunits) {
 ...
 */
export function lineWeightToDrawingUnits(lineWeight, insunits) {
    var mm = Math.max(0.01, lineWeight / 100);
    var perUnit = mmPerDrawingUnit(insunits);
    if (perUnit <= 0) {
        return mm;
    }
    return mm / perUnit;
...
//# sourceMappingURL=AcPdfUnits.js.map