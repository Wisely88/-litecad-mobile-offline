...
var AcEdCommandLine = /** @class */ (function () {
    function AcEdCommandLine(container) {
        ...
        this.recentMessages = [];
        this.isCommandLifecycleBound = false;
    ...
    AcEdCommandLine.prototype.setInputReadOnly = function (readOnly) {
     ...
     */
    AcEdCommandLine.prototype.showMessage = function (message, type, msgKey) {
        if (type === void 0) { type = 'info'; }
        this.appendMessage(message, type, msgKey);
    };
    AcEdCommandLine.prototype.cancelActiveSession = function () {
    ...
    AcEdCommandLine.prototype.executeCommand = function (cmdLine) {
        if (!cmdLine || !cmdLine.trim()) {
            if (this.lastExecuted)
                ...
                this.showMessage(this.localize('main.commandLine.noLast', '(no last command)'));
                return;
            }
        }
        ...
        if (!command) {
            ...
            this.showMessage("".concat(unknown, ": ").concat(cmdLine), 'warning');
            return;
        }
        ...
        this.showMessage("".concat(executed, ": ").concat(command.localName));
    ...
    AcEdCommandLine.prototype.handleInputChange = function () {
        ...
        if (matches.length) {
            ...
            matches.forEach(function (item, idx) {
                ...
                var aliases = AcApDocManager.instance.commandManager.getCommandAliases(item.command, item.commandGroup);
                var aliasText = aliases.length ? "(".concat(aliases.join(', '), ")") : '';
                div.innerHTML = "<strong>".concat(item.command.globalName).concat(aliasText, " - ").concat(description, "</strong>");
    ...
    AcEdCommandLine.prototype.renderKeywordPrompt = function (options, onClick) {
        ...
        var defaultText = (_c = promptFormat.defaultKeyword) !== null && _c !== void 0 ? _c : options.valueDefaultDisplayText;
        if (defaultText) {
            this.promptEl.append(" <".concat(defaultText, ">"));
        }
        this.promptEl.append(': ');
        this.recordRecentMessage((_d = this.promptEl.textContent) !== null && _d !== void 0 ? _d : options.message);
    };
    ...
    AcEdCommandLine.prototype.bindCommandLifecycleEvents = function () {
        var _this = this;
        if (this.isCommandLifecycleBound)
            return;
        var editorEvents;
        try {
            ...
            setTimeout(function () { return _this.bindCommandLifecycleEvents(); }, 0);
            return;
        }
        editorEvents.commandWillStart.addEventListener(function () {
            _this.cancelRecentAutoHide();
            _this.showRecentPanel();
        });
        editorEvents.commandEnded.addEventListener(function () {
            _this.scheduleRecentAutoHide();
        });
    ...
    AcEdCommandLine.prototype.appendMessage = function (msg, type, msgKey) {
        if (type === void 0) { type = 'info'; }
    ...
    AcEdCommandLine.prototype.recordRecentMessage = function (message) {
        var normalized = message === null || message === void 0 ? void 0 : message.replace(/\s+/g, ' ').trim();
        if (!normalized)
            return;
    ...
    AcEdCommandLine.prototype.renderRecentMessages = function () {
        var _this = this;
        this.recentPanel.innerHTML = '';
        this.recentMessages.forEach(function (message) {
            var line = document.createElement('div');
            line.className = 'ml-cli-recent-line';
            line.textContent = message;
            _this.recentPanel.appendChild(line);
        });
    };
    /** Start a 2-second timer to hide recent messages after command ends. */
    AcEdCommandLine.prototype.scheduleRecentAutoHide = function () {
        var _this = this;
        this.cancelRecentAutoHide();
        this.recentHideTimer = setTimeout(function () {
            _this.hideRecentPanel();
            _this.recentHideTimer = undefined;
        }, 2000);
    };
    /** Cancel current auto-hide timer when a new command starts. */
    AcEdCommandLine.prototype.cancelRecentAutoHide = function () {
        if (!this.recentHideTimer)
            return;
        clearTimeout(this.recentHideTimer);
        this.recentHideTimer = undefined;
    };
    /** Show recent message area. */
    AcEdCommandLine.prototype.showRecentPanel = function () {
        this.recentPanel.classList.remove('hidden');
    };
    /** Hide recent message area. */
    AcEdCommandLine.prototype.hideRecentPanel = function () {
