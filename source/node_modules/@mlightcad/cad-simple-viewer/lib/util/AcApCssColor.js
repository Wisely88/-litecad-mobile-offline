 ...
 */
export function preferExactAciColor(color) {
    if (!color.isByColor)
        return color;
    var rgb = color.RGB;
    if (rgb == null)
        return color;
    var index = AcCmColorUtil.getIndexByColor(rgb);
    if (index == null)
        return color;
    var aci = new AcCmColor();
 ...
 */
function shouldSkipFromString(css) {
    var s = css.trim();
 ...
 */
export function parseCssToAcCmColor(css, fallbackRgb) {
    if (fallbackRgb === void 0) { fallbackRgb = [123, 135, 148]; }
    var trimmed = css.trim();
    if (!trimmed) {
        var empty = new AcCmColor();
        empty.setRGB(fallbackRgb[0], fallbackRgb[1], fallbackRgb[2]);
        return empty;
    }
    if (!shouldSkipFromString(trimmed)) {
        try {
            var fromString = AcCmColor.fromString(trimmed);
            if (fromString)
                return preferExactAciColor(fromString);
        }
        catch (_a) {
    ...
    try {
        ...
        var fallback = new AcCmColor();
        fallback.setRGB(fallbackRgb[0], fallbackRgb[1], fallbackRgb[2]);
        return fallback;
    }
}
/** Stable `#rrggbb` for sidecar JSON (avoids `rgb()` round-trip issues). */
export function acCmColorToCssHex(color) {
    var byte = function (n) {
        return Math.max(0, Math.min(255, Math.round(n !== null && n !== void 0 ? n : 0)))
            .toString(16)
            .padStart(2, '0');
    };
    return "#".concat(byte(color.red)).concat(byte(color.green)).concat(byte(color.blue));
}
//# sourceMappingURL=AcApCssColor.js.map