...
import { AcGeBox2d, AcGePoint3d, FLOAT_TOL } from '@mlightcad/data-model';
...
var AcApZoomCmd = /** @class */ (function (_super) {
     ...
     */
    AcApZoomCmd.rememberOriginalView = function (view, layoutBtrId) {
        if (!layoutBtrId || view.width <= 0 || view.height <= 0)
            return;
        if (AcApZoomCmd.originalViewBoxByLayout.has(layoutBtrId))
            return;
        AcApZoomCmd.originalViewBoxByLayout.set(layoutBtrId, AcApZoomCmd.captureViewBoxFromView(view));
    };
    /**
     ...
     */
    AcApZoomCmd.clearOriginalViews = function () {
     ...
     */
    AcApZoomCmd.captureViewBoxFromView = function (view) {
    ...
    AcApZoomCmd.prototype.applyRelativeScale = function (context, factor, centerPoint) {
        ...
        var width = Math.max(current.max.x - current.min.x, FLOAT_TOL);
        var height = Math.max(current.max.y - current.min.y, FLOAT_TOL);
     ...
     * @param context - Current command context.
     */
    AcApZoomCmd.prototype.runOriginal = function (context) {
        var layoutBtrId = context.doc.database.currentSpaceId;
        var original = layoutBtrId
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        ...
                        firstPrompt.keywords.add(AcApI18n.t('jig.zoom.keywords.original.display'), AcApI18n.t('jig.zoom.keywords.original.global'), AcApI18n.t('jig.zoom.keywords.original.local'));
     ...
     */
    AcApZoomCmd.originalViewBoxByLayout = new Map();
    return AcApZoomCmd;
}(AcEdCommand));
export { AcApZoomCmd };
