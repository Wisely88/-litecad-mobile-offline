...
import { AcDbArc, AcGeCircArc2d, AcGeMathUtil, AcGePoint3d } from '@mlightcad/data-model';
 ...
 */
function toArcDefinition(arc) {
    if (!arc)
        return undefined;
    var normalSign = arc.clockwise ? -1 : 1;
    var center = { x: arc.center.x, y: arc.center.y, z: 0 };
    return {
        center: center,
        radius: arc.radius,
        startAngle: angleByNormalSign(center, { x: arc.startPoint.x, y: arc.startPoint.y, z: 0 }, normalSign),
        endAngle: angleByNormalSign(center, { x: arc.endPoint.x, y: arc.endPoint.y, z: 0 }, normalSign),
...
function createArcFromThreePoints(start, second, end, reverseDirection) {
    ...
    return toArcDefinition(AcGeCircArc2d.tryCreateByThreePoints(start, second, end, reverseDirection));
}
/**
 * Builds arc by center/start/end with explicit orientation.
 */
function createArcFromCenterStartEnd(center, start, end, normalSign) {
    return toArcDefinition(AcGeCircArc2d.tryCreateByCenterStartEnd(center, start, end, normalSign === -1));
}
/**
...
function createArcFromCenterStartProjectedEnd(center, start, rawEnd, normalSign) {
    return toArcDefinition(AcGeCircArc2d.tryCreateByCenterStartProjectedEnd(center, start, rawEnd, normalSign === -1));
}
/**
...
function createArcFromCenterStartSweep(center, start, sweepRad) {
    return toArcDefinition(AcGeCircArc2d.tryCreateByCenterStartSweep(center, start, sweepRad));
}
/**
 * Builds center-start arc from chord length.
 */
function createArcFromCenterStartChord(center, start, chordLength) {
    return toArcDefinition(AcGeCircArc2d.tryCreateByCenterStartChord(center, start, chordLength));
}
/**
 * Builds start-end arc from included angle.
 */
function createArcFromStartEndAngle(start, end, sweepRad) {
    return toArcDefinition(AcGeCircArc2d.tryCreateByStartEndAngle(start, end, sweepRad));
}
/**
 * Builds start-end arc from tangent direction at start.
 */
function createArcFromStartEndDirection(start, end, directionRad) {
    return toArcDefinition(AcGeCircArc2d.tryCreateByStartEndDirection(start, end, directionRad));
}
/**
...
function createArcFromStartEndRadius(start, end, radiusInput) {
    return toArcDefinition(AcGeCircArc2d.tryCreateByStartEndRadius(start, end, radiusInput));
}
/**
