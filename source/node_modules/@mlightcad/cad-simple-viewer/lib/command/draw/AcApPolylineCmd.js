...
import { AcDbPolyline, AcGePoint2d, AcGePoint3d, AcGeTol, TAU } from '@mlightcad/data-model';
...
var AcApPolylineCmd = /** @class */ (function (_super) {
     ...
     */
    AcApPolylineCmd.prototype.complementBulge = function (bulge) {
        if (AcGeTol.equalToZero(bulge))
            return bulge;
        var sweep = 4 * Math.atan(bulge);
        var sign = Math.sign(sweep) || 1;
        var complementSweep = sign * (Math.PI * 2 - Math.abs(sweep));
        if (AcGeTol.equalToZero(complementSweep))
            return bulge;
     ...
     */
    AcApPolylineCmd.prototype.flipArcBulgeForCtrl = function (bulge) {
        return -this.complementBulge(bulge);
    };
    /**
     * Arc center from chord endpoints and bulge (polyline arc convention).
     */
    AcApPolylineCmd.prototype.computeArcCenterFromBulge = function (start, end, bulge) {
     ...
     */
    AcApPolylineCmd.prototype.getArcTangentAtEnd = function (start, end, bulge) {
        var center = this.computeArcCenterFromBulge(start, end, bulge);
        if (!center)
            return undefined;
     ...
     */
    AcApPolylineCmd.prototype.getIncomingTangentRadians = function (points, bulges) {
        if (points.length < 2)
            return 0;
        var start = points[points.length - 1];
        var prev = points[points.length - 2];
        var prevBulge = bulges[points.length - 2];
        if (prevBulge == null || AcGeTol.equalToZero(prevBulge)) {
            return Math.atan2(start.y - prev.y, start.x - prev.x);
        }
        var exitTangent = this.getArcTangentAtEnd(prev, start, prevBulge);
        return exitTangent !== null && exitTangent !== void 0 ? exitTangent : Math.atan2(start.y - prev.y, start.x - prev.x);
    };
    /**
     * Bulge for an arc through `start` and `end` that is tangent to `tangentRad` at `start`.
     */
    AcApPolylineCmd.prototype.computeBulgeFromTangentDirection = function (start, end, tangentRad) {
        var tx = Math.cos(tangentRad);
        var ty = Math.sin(tangentRad);
        var dx = end.x - start.x;
        var dy = end.y - start.y;
        var nx = -ty;
        var ny = tx;
     ...
     */
    AcApPolylineCmd.prototype.computeBulgeFromCenterWithTangent = function (start, end, center, tangentRad) {
        ...
        var crossStartTangent = v1x * ty - v1y * tx;
        var ccw;
        if (AcGeTol.isPositive(Math.abs(crossStartTangent))) {
            ccw = crossStartTangent > 0;
        }
        else {
            var chordCrossTangent = chordX * ty - chordY * tx;
            ccw = chordCrossTangent >= 0;
        }
        var startAngle = Math.atan2(v1y, v1x);
        var endAngle = Math.atan2(v2y, v2x);
        var sweep;
        if (ccw) {
     ...
     */
    AcApPolylineCmd.prototype.computeBulgeFromTangentAndIncludedAngle = function (start, end, tangentRad, includedAngleRad) {
        var tx = Math.cos(tangentRad);
        var ty = Math.sin(tangentRad);
        var dx = end.x - start.x;
        var dy = end.y - start.y;
        ...
        var referenceBulge = this.computeBulgeFromCenterWithTangent(start, end, center, tangentRad);
        if (referenceBulge == null)
            return undefined;
        var requested = Math.tan(includedAngleRad / 4);
    ...
    AcApPolylineCmd.prototype.execute = function (context) {
        return __awaiter(this, void 0, void 0, function () {
            var points, bulges, closed, applyArcDirection, computeBulgeFromCenter, computeBulgeFromThreePoints, computeBulgeFromRadius, getIncomingTangent, computeDefaultArcBulge, computeBulgeFromTangentAndIncludedAngle, createStaticJig, createPreviewJig, getCurrentPoint, setSegmentBulge, appendPoint, undoLast, shouldFinish, LineState, ArcState, firstPointPrompt, firstPointResult, machine, db, polyline_1;
            var _this = this;
                ...
                switch (_a.label) {
                    case 0:
                        ...
                        applyArcDirection = function (bulge) {
                            ...
                            return toggles.ctrlArcFlip ? _this.flipArcBulgeForCtrl(bulge) : bulge;
                        };
                        ...
                        getIncomingTangent = function () {
                            return _this.getIncomingTangentRadians(points, bulges);
                        };
                        computeDefaultArcBulge = function (end) {
                            var start = getCurrentPoint();
                            if (!start)
                                return undefined;
                            return _this.computeBulgeFromTangentDirection(start, end, getIncomingTangent());
                        };
                        computeBulgeFromTangentAndIncludedAngle = function (start, end, tangentRad, includedAngleRad) {
                            return _this.computeBulgeFromTangentAndIncludedAngle(start, end, tangentRad, includedAngleRad);
                        };
                        ...
                        ArcState = /** @class */ (function () {
                            ...
                            ArcState.prototype.buildPrompt = function () {
                                ...
                                prompt.jig = createPreviewJig(undefined, function (end) {
                                    return applyArcDirection(computeDefaultArcBulge(end));
                                });
                                prompt.allowNone = true;
                                return prompt;
                            };
                            ArcState.prototype.handleResult = function (result) {
                                return __awaiter(this, void 0, void 0, function () {
                                    var bulge, keyword, startPoint, anglePrompt, angleResult, endPrompt, angleRad_1, tangent_1, endResult, angleBulge, centerPrompt, centerResult_1, endPrompt, endResult, bulge, secondPrompt, secondResult_1, endPrompt, endResult, bulge, radiusPrompt, radiusResult_1, endPrompt, endResult, bulge;
                                    var _a, _b, _c;
                                    return __generator(this, function (_d) {
                                        switch (_d.label) {
                                            case 0:
                                                if (result.status === AcEdPromptStatus.OK) {
                                                    bulge = computeDefaultArcBulge(result.value);
                                                    if (bulge === undefined)
                                                        return [2 /*return*/, 'continue'];
                                            ...
                                            case 1:
                                                ...
                                                angleRad_1 = ((_b = angleResult.value) !== null && _b !== void 0 ? _b : 0) * (Math.PI / 180);
                                                tangent_1 = getIncomingTangent();
                                                endPrompt.jig = createPreviewJig(undefined, function (end) {
                                                    return applyArcDirection(computeBulgeFromTangentAndIncludedAngle(startPoint, end, tangent_1, angleRad_1));
                                                });
                                            ...
                                            case 2:
                                                ...
                                                if (shouldFinish(endResult.status))
                                                    return [2 /*return*/, 'finish'];
                                                angleBulge = computeBulgeFromTangentAndIncludedAngle(startPoint, endResult.value, tangent_1, angleRad_1);
                                                if (angleBulge === undefined)
                                                    return [2 /*return*/, 'continue'];
