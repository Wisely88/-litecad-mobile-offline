 ...
 */
export function acapSetDocsBaseUrl(url) {
 ...
 */
export function acapDocsUrl(docPath, locale) {
    if (locale === void 0) { locale = 'en'; }
    var base = acapGetDocsBaseUrl();
    var lang = locale.toLowerCase().split(/[-_]/)[0];
    var prefix = !lang || lang === 'en' ? '' : "/".concat(lang);
    var path = docPath.replace(/^\/+/, '');
    return "".concat(base).concat(prefix, "/").concat(path);
}
/**
 ...
 */
export function normalizeDocsBaseUrl(url) {
    var trimmed = url.trim();
    if (!trimmed)
        return ACAP_DEFAULT_DOCS_BASE_URL.replace(/\/+$/, '');
    return trimmed.replace(/\/+$/, '');
}
//# sourceMappingURL=AcApDocsUrl.js.map