...
import { AcTrBaseView, acTrCssRectToWcsBox, acTrIntersectCssRects, AcTrOverlayViewport, acTrWcsBoxToCssRect } from '@mlightcad/three-renderer';
...
var AcTrLayoutView = /** @class */ (function (_super) {
    ...
    function AcTrLayoutView(renderer, layoutBtrId, width, height) {
        ...
        _this._overlayViewport = new AcTrOverlayViewport(renderer);
        return _this;
    }
    ...
    Object.defineProperty(AcTrLayoutView.prototype, "viewportViews", {
        /**
    ...
    Object.defineProperty(AcTrLayoutView.prototype, "overlayViewport", {
        /**
    ...
    Object.defineProperty(AcTrLayoutView.prototype, "viewportsBoundingBox", {
        /**
        ...
        get: function () {
            ...
            this._viewportViews.forEach(function (vp) {
                var vpBox = vp.viewport.box;
    ...
    AcTrLayoutView.prototype.render = function (scene) {
        ...
        var needsRedraw = this._renderer.render(scene.internalScene, this._camera);
     ...
     */
    AcTrLayoutView.prototype.drawOverlay = function (scene) {
        var overlay = this._overlayViewport;
        if (!overlay.visible)
            return;
        overlay.render(scene.internalScene, this._height);
        var modelSpaceLayout = scene.modelSpaceLayout;
        if (!modelSpaceLayout || this._viewportViews.size === 0)
            return;
     ...
     */
    AcTrLayoutView.prototype.drawOverlayViewports = function (modelScene, overlay) {
        var _this = this;
        var loupe = overlay.screenRect;
        var viewBox = overlay.viewBox;
        var visibility = modelScene.visible;
        modelScene.visible = true;
        this._viewportViews.forEach(function (viewportView) {
            var magRect = acTrWcsBoxToCssRect(viewportView.viewport.box, viewBox, loupe);
            var hit = acTrIntersectCssRects(magRect, loupe);
            if (!hit)
                return;
            var paperHit = acTrCssRectToWcsBox(hit, viewBox, loupe);
            var modelBox = new AcGeBox2d();
            modelBox.expandByPoint(viewportView.paperPointToModel({ x: paperHit.minX, y: paperHit.minY }));
            modelBox.expandByPoint(viewportView.paperPointToModel({ x: paperHit.maxX, y: paperHit.minY }));
            modelBox.expandByPoint(viewportView.paperPointToModel({ x: paperHit.maxX, y: paperHit.maxY }));
            modelBox.expandByPoint(viewportView.paperPointToModel({ x: paperHit.minX, y: paperHit.maxY }));
            if (modelBox.isEmpty())
                return;
            overlay.renderNested(modelScene, modelBox, hit, hit, _this._height, viewportView.viewport.viewTwistAngle);
        });
