...
var AcGePolyline2d = /** @class */ (function (_super) {
    ...
    Object.defineProperty(AcGePolyline2d.prototype, "vertices", {
        /**
    ...
    Object.defineProperty(AcGePolyline2d.prototype, "area", {
        /**
         * The area enclosed by this polyline. Open polylines return `0`.
         */
        get: function () {
            if (!this._closed || this._vertices.length < 3)
                return 0;
     ...
     */
    AcGePolyline2d.prototype.removeVertexAt = function (index) {
        if (index < 0 || index >= this._vertices.length) {
            throw new Error("Index ".concat(index, " is out of bounds. Valid range is 0 to ").concat(this._vertices.length - 1, "."));
        }
     ...
     */
    AcGePolyline2d.prototype.reset = function (reuse, numVerts) {
        if (reuse) {
     ...
     * @inheritdoc
     */
    AcGePolyline2d.prototype.transform = function (matrix) {
        var flipBulge = matrix.determinant() < 0;
        this._vertices.forEach(function (vertex) {
            var transformedPoint = new AcGePoint2d(vertex).applyMatrix2d(matrix);
     ...
     */
    AcGePolyline2d.prototype.clone = function () {
        return new AcGePolyline2d(this._vertices.map(function (vertex) { return (__assign({}, vertex)); }), this._closed);
    };
    /**
     ...
     */
    AcGePolyline2d.prototype.tessellate = function (options) {
        var points = [];
        var length = this._vertices.length;
        for (var index = 0; index < length; ++index) {
            var vertex = this._vertices[index];
            if (vertex.bulge) {
                ...
                if (nextVertex) {
                    var arc = new AcGeCircArc2d(vertex, nextVertex, vertex.bulge);
                    var arcPoints = arc.tessellate(options);
     ...
     */
    AcGePolyline2d.prototype.offset = function (offsetDist) {
        return acgeOffsetPolyline2d(this, offsetDist);
    };
    return AcGePolyline2d;
}(AcGeCurve2d));
export { AcGePolyline2d };
function pushUniquePoint2d(points, candidate) {
    var last = points[points.length - 1];
    if (last) {
        var dx = last.x - candidate.x;
        var dy = last.y - candidate.y;
