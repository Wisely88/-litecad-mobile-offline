...
import { acgeIsNonZeroDirection, acgeNormalizeSplineWeights, acgeResolveControlPointSplineDegree, acgeResolveFitPointSplineDegree } from './AcGeSplineUtil';
var AcGeSpline3d = /** @class */ (function (_super) {
    ...
    Object.defineProperty(AcGeSpline3d.prototype, "isFitPointSpline", {
        /**
    ...
    Object.defineProperty(AcGeSpline3d.prototype, "isControlPointSpline", {
        /**
     ...
     */
    AcGeSpline3d.prototype.getGripPoints = function () {
        if (this.isFitPointSpline) {
            return this._fitPoints.map(function (point) { var _a; return new AcGePoint3d(point.x, point.y, (_a = point.z) !== null && _a !== void 0 ? _a : 0); });
        }
        return this._controlPoints.map(function (point) { var _a; return new AcGePoint3d(point.x, point.y, (_a = point.z) !== null && _a !== void 0 ? _a : 0); });
    };
    /**
     ...
     */
    AcGeSpline3d.prototype.moveGripPointsAt = function (indices, offset) {
        if (this.isFitPointSpline) {
     ...
     */
    AcGeSpline3d.prototype.moveFitPointsAt = function (indices, offset) {
        var e_1, _a;
        if (!this._fitPoints) {
            return;
        }
        try {
            for (var indices_1 = __values(indices), indices_1_1 = indices_1.next(); !indices_1_1.done; indices_1_1 = indices_1.next()) {
                ...
                var point = this._fitPoints[index];
                if (point) {
     ...
     * @param offset - Translation applied in WCS.
     */
    AcGeSpline3d.prototype.moveControlPointsAt = function (indices, offset) {
        var e_2, _a;
        try {
            for (var indices_2 = __values(indices), indices_2_1 = indices_2.next(); !indices_2_1.done; indices_2_1 = indices_2.next()) {
                var index = indices_2_1.value;
                if (!Number.isInteger(index) || index < 0) {
                    continue;
                }
                var point = this.getControlPointAt(index);
                if (point) {
                    AcGeGeometryUtil.applyOffsetToPoint3d(point, offset);
                }
            }
     ...
     */
    AcGeSpline3d.prototype.rebuildAfterControlPointMutation = function () {
        var knots = this._nurbsCurve.knots();
        var weights = this._nurbsCurve.weights();
        if (this.isFitPointSpline) {
     ...
     */
    AcGeSpline3d.prototype.tessellate = function (options) {
        var _this = this;
        var _a, _b;
        var resolved = AcGeCircArc2d.resolveTessellateOptions(options);
        var knots = this._nurbsCurve.knots();
        var degree = this._nurbsCurve.degree();
        ...
        var startParam = knots[degree];
        var endParam = knots[knots.length - degree - 1];
        var n = resolved.circleSides;
        var length = Math.abs(this.length);
        var deviation = (_a = resolved.deviation) !== null && _a !== void 0 ? _a : Math.max(FLOAT_TOL, length <= FLOAT_TOL ? FLOAT_TOL : length / (2 * n * n));
        return AcGeCurve3d.tessellateParametricCurve(startParam, endParam, function (t) { return _this.evaluateAt(t); }, {
            deviation: deviation,
            minSegments: (_b = resolved.minSegments) !== null && _b !== void 0 ? _b : (this.closed ? 8 : 2),
    ...
    AcGeSpline3d.prototype.clone = function () {
        if (this._fitPoints && this._knotParameterization) {
            return new AcGeSpline3d(this._fitPoints.map(AcGeGeometryUtil.point2dToPoint3d), this._knotParameterization, this._degree, this._closed, this._startTangent
                ? AcGeGeometryUtil.point2dToPoint3d(this._startTangent)
                : undefined, this._endTangent
    ...
    AcGeSpline3d.fromControlPoints = function (controlPoints, knots, weights, declaredDegree, closed) {
        ...
        var degree = acgeResolveControlPointSplineDegree(declaredDegree, controlPoints.length, knots.length);
        if (controlPoints.length < degree + 1) {
            return null;
        }
        try {
            return new AcGeSpline3d(controlPoints, knots, acgeNormalizeSplineWeights(weights, controlPoints.length), degree, closed);
        }
        catch (_a) {
            return null;
        }
    ...
    AcGeSpline3d.fromFitPoints = function (fitPoints, knotParam, declaredDegree, closed, startTangent, endTangent) {
        ...
        var hasStartTangent = acgeIsNonZeroDirection(startTangent);
        var hasEndTangent = acgeIsNonZeroDirection(endTangent);
        var tangentCount = (hasStartTangent ? 1 : 0) + (hasEndTangent ? 1 : 0);
        var degree = acgeResolveFitPointSplineDegree(declaredDegree, fitPoints.length, tangentCount);
    ...
    AcGeSpline3d.fromDwgSplineEdge = function (spline) {
        ...
        if (spline.numberOfFitData > 0) {
            ...
            return AcGeSpline3d.fromFitPoints(fitPoints, 'Uniform', spline.degree, false, spline.startTangent
                ? AcGeGeometryUtil.point2dToPoint3d(spline.startTangent)
                : undefined, spline.endTangent
