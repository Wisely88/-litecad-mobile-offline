var __extends = (this && this.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (Object.prototype.hasOwnProperty.call(b, p)) d[p] = b[p]; };
        return extendStatics(d, b);
    };
    return function (d, b) {
        if (typeof b !== "function" && b !== null)
            throw new TypeError("Class extends value " + String(b) + " is not a constructor or null");
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
var __assign = (this && this.__assign) || function () {
    __assign = Object.assign || function(t) {
        for (var s, i = 1, n = arguments.length; i < n; i++) {
            s = arguments[i];
            for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p))
                t[p] = s[p];
        }
        return t;
    };
    return __assign.apply(this, arguments);
};
var __values = (this && this.__values) || function(o) {
    var s = typeof Symbol === "function" && Symbol.iterator, m = s && o[s], i = 0;
    if (m) return m.call(o);
    if (o && typeof o.length === "number") return {
        next: function () {
            if (o && i >= o.length) o = void 0;
            return { value: o && o[i++], done: !o };
        }
    };
    throw new TypeError(s ? "Object is not iterable." : "Symbol.iterator is not defined.");
};
import { AcCmErrors } from '@mlightcad/common';
import { AcGeBox3d, AcGePlane, AcGePoint3d, AcGeVector3d } from '../math';
import { AcGeMathUtil, FLOAT_TOL, ORIGIN_POINT_3D, TAU } from '../util';
import { AcGeTol } from '../util/AcGeTol';
import { AcGeCircArc2d } from './AcGeCircArc2d';
import { AcGeCircArc3d } from './AcGeCircArc3d';
import { AcGeCurve3d } from './AcGeCurve3d';
/**
 * Class representing a 3d ellipse arc defined by center, normal, majorAxis, majorAxisRadius,
 * minorAxisRadius, startAngle, and endAngle.
 * - The majorAxis vector represents half the major axis of the ellipse (that is, from the center
 * point to the start point of the ellipse) and is the zero angle for startAngle and endAngle.
 * - Positive angles are counter-clockwise when looking down the normal vector (that is, right-hand
 * rule). A startAngle of 0 and endAngle of 2pi will produce a closed Ellipse.
 * - If startAngle is equal to 0 and endAngle is equal to 2 * Math.PI, it represents a full 3d ellipse.
 */
var AcGeEllipseArc3d = /** @class */ (function (_super) {
    __extends(AcGeEllipseArc3d, _super);
    /**
     * Construct an instance of the ellipse arc.
     * @param center Center point of the ellipse.
     * @param normal Normal vector defining the plane of the ellipse
     * @param majorAxis Major axis vector (in WCS coordinates) of the ellipse.
     * @param majorAxisRadius Major axis radius of the ellipse.
     * @param minorAxisRadius Minor axis radius of the ellipse.
     * @param startAngle Start angle of the ellipse arc in radians.
     * @param endAngle End angle of the ellipse arc in radians.
     */
    function AcGeEllipseArc3d(center, normal, majorAxis, majorAxisRadius, minorAxisRadius, startAngle, endAngle) {
        if (startAngle === void 0) { startAngle = 0; }
        if (endAngle === void 0) { endAngle = TAU; }
        var _this = _super.call(this) || this;
        _this.center = center;
        _this.normal = normal;
        _this.majorAxis = majorAxis;
        _this.majorAxisRadius = majorAxisRadius;
        _this.minorAxisRadius = minorAxisRadius;
        // Check whether it is a full ellipse
        var angleDiff = Math.abs(endAngle - startAngle);
        if (Math.abs(angleDiff - TAU) < 1e-10 ||
            Math.abs(angleDiff - 2 * TAU) < 1e-10) {
            _this.startAngle = 0;
            _this.endAngle = TAU;
        }
        else {
            _this.startAngle = startAngle;
            _this.endAngle = endAngle;
        }
        return _this;
    }
    Object.defineProperty(AcGeEllipseArc3d.prototype, "center", {
        /**
         * Center of the ellipse in 3d space
         */
        get: function () {
            return this._center;
        },
        set: function (value) {
            this._center = new AcGePoint3d(value.x, value.y, value.z || 0);
            this._boundingBoxNeedsUpdate = true;
        },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(AcGeEllipseArc3d.prototype, "majorAxisRadius", {
        /**
         * Major axis radius of the ellipse
         */
        get: function () {
            return this._majorAxisRadius;
        },
        set: function (value) {
            if (value < 0)
                throw AcCmErrors.ILLEGAL_PARAMETERS;
            this._majorAxisRadius = value;
            this._boundingBoxNeedsUpdate = true;
        },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(AcGeEllipseArc3d.prototype, "minorAxisRadius", {
        /**
         * Minor axis radius of the ellipse
         */
        get: function () {
            return this._minorAxisRadius;
        },
        set: function (value) {
            if (value < 0)
                throw AcCmErrors.ILLEGAL_PARAMETERS;
            this._minorAxisRadius = value;
            this._boundingBoxNeedsUpdate = true;
        },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(AcGeEllipseArc3d.prototype, "startAngle", {
        /**
         * Start angle of the ellipse arc in radians in the range -pi to pi.
         */
        get: function () {
            return this._startAngle;
        },
        set: function (value) {
            this._startAngle = AcGeMathUtil.normalizeAngle(value);
            this._boundingBoxNeedsUpdate = true;
        },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(AcGeEllipseArc3d.prototype, "endAngle", {
        /**
         * End angle of the ellipse arc in radians in the range -pi to pi.
         */
        get: function () {
            return this._endAngle;
        },
        set: function (value) {
            this._endAngle =
                this.startAngle == 0 && value == TAU
                    ? value
                    : AcGeMathUtil.normalizeAngle(value);
            this._boundingBoxNeedsUpdate = true;
        },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(AcGeEllipseArc3d.prototype, "deltaAngle", {
        /**
         * Return angle between endAngle and startAngle in range 0 to 2*PI
         */
        get: function () {
            var diff = this.endAngle - this.startAngle;
            // Handle full circle case
            if (Math.abs(diff - TAU) < 1e-10) {
                return TAU;
            }
            return AcGeMathUtil.normalizeAngle(diff);
        },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(AcGeEllipseArc3d.prototype, "isLargeArc", {
        /**
         * Return true if the arc is a large arc whose delta angle value is greater than PI.
         */
        get: function () {
            return Math.abs(this.deltaAngle) > Math.PI ? 1 : 0;
        },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(AcGeEllipseArc3d.prototype, "clockwise", {
        /**
         * Return true if the arc is clockwise from startAngle to endAngle
         */
        get: function () {
            return this.deltaAngle <= 0;
        },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(AcGeEllipseArc3d.prototype, "normal", {
        /**
         * Unit normal vector defining the plane of the ellipse
         */
        get: function () {
            return this._normal;
        },
        set: function (value) {
            this._normal = new AcGeVector3d(value.x, value.y, value.z);
            // Normalize the normal vector to ensure it's a unit vector
            this._normal.normalize();
            this._boundingBoxNeedsUpdate = true;
        },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(AcGeEllipseArc3d.prototype, "majorAxis", {
        /**
         * Unit major axis vector (in WCS coordinates) of the ellipse. The major axis vector is the vector from the
         * ellipse's center point to its start point.
         */
        get: function () {
            return this._majorAxis;
        },
        set: function (value) {
            this._majorAxis = new AcGeVector3d(value.x, value.y, value.z);
            // Normalize the normal vector to ensure it's a unit vector
            this._majorAxis.normalize();
            this._boundingBoxNeedsUpdate = true;
        },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(AcGeEllipseArc3d.prototype, "minorAxis", {
        /**
         * Unit minor axis vector (in WCS coordinates) of the ellipse.
         */
        get: function () {
            return new AcGeVector3d()
                .crossVectors(this.normal, this.majorAxis)
                .normalize();
        },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(AcGeEllipseArc3d.prototype, "startPoint", {
        /**
         * Compute the start point of the ellipse arc.
         * @returns Return the start point of the ellipse arc.
         */
        get: function () {
            return this.getPointAtAngle(this._startAngle);
        },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(AcGeEllipseArc3d.prototype, "endPoint", {
        /**
         * Compute the end point of the ellipse arc.
         * @returns Return the end point of the ellipse arc.
         */
        get: function () {
            return this.getPointAtAngle(this._endAngle);
        },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(AcGeEllipseArc3d.prototype, "midPoint", {
        /**
         * Compute the midpoint of the ellipse arc.
         * The midpoint is defined at the middle parameter angle
         * (not arc-length midpoint).
         */
        get: function () {
            var startAngle = this.startAngle;
            var deltaAngle = this.deltaAngle;
            // Closed ellipse: midpoint at PI
            if (this.closed || Math.abs(deltaAngle - TAU) < 1e-10) {
                startAngle = 0;
                deltaAngle = TAU;
            }
            var midAngle = startAngle + deltaAngle / 2;
            return this.getPointAtAngle(midAngle);
        },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(AcGeEllipseArc3d.prototype, "isCircular", {
        /**
         * @inheritdoc
         */
        /**
         * Check if this ellipse arc is actually a circular arc (major and minor radii are equal)
         * @returns True if the ellipse arc is circular
         */
        get: function () {
            return AcGeTol.equal(this.majorAxisRadius, this.minorAxisRadius);
        },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(AcGeEllipseArc3d.prototype, "length", {
        get: function () {
            // For circular arcs, use the exact formula: length = radius * deltaAngle
            if (this.isCircular) {
                return this.majorAxisRadius * Math.abs(this.deltaAngle);
            }
            // For elliptical arcs, use numerical integration
            var steps = 1000;
            var step = this.deltaAngle / steps;
            var length = 0;
            var prevPoint = this.getPointAtAngle(this.startAngle);
            for (var i = 1; i <= steps; i++) {
                var angle = this.startAngle + i * step;
                var point = this.getPointAtAngle(angle);
                var dx = point.x - prevPoint.x;
                var dy = point.y - prevPoint.y;
                var dz = point.z - prevPoint.z;
                length += Math.sqrt(dx * dx + dy * dy + dz * dz);
                prevPoint = point;
            }
            return length;
        },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(AcGeEllipseArc3d.prototype, "area", {
        /**
         * The area enclosed by this curve. Open ellipse arcs return `0`.
         */
        get: function () {
            if (Math.abs(this.deltaAngle - TAU) >= 1e-10)
                return 0;
            return Math.PI * this.majorAxisRadius * this.minorAxisRadius;
        },
        enumerable: false,
        configurable: true
    });
    /**
     * Compute the bounding box of the 3D ellipse arc.
     * The bounding box is defined by its minimum and maximum x, y, and z coordinates.
     * @returns Return bounding box containing the min and max coordinates
     */
    AcGeEllipseArc3d.prototype.calculateBoundingBox = function () {
        var e_1, _a;
        // To make it faster, check whether major axis is euqal to x-axis or y-axis firstly because
        // major axis of ellipse arc is equal to x-axis or y-axis in most of cases.
        if (this.majorAxis.equals(AcGeVector3d.X_AXIS) ||
            this.majorAxis.equals(AcGeVector3d.Y_AXIS) ||
            this.majorAxis.isParallelTo(AcGeVector3d.X_AXIS) ||
            this.majorAxis.isParallelTo(AcGeVector3d.Y_AXIS)) {
            var angles = [this.startAngle, this.endAngle];
            for (var i = 0; i < 2 * Math.PI; i += Math.PI / 2) {
                if (AcGeMathUtil.isBetweenAngle(i, this.startAngle, this.endAngle)) {
                    angles.push(i);
                }
            }
            var minX = Infinity, minY = Infinity, minZ = Infinity;
            var maxX = -Infinity, maxY = -Infinity, maxZ = -Infinity;
            try {
                for (var angles_1 = __values(angles), angles_1_1 = angles_1.next(); !angles_1_1.done; angles_1_1 = angles_1.next()) {
                    var angle = angles_1_1.value;
                    var point = this.getPointAtAngle(angle);
                    if (point.x < minX)
                        minX = point.x;
                    if (point.y < minY)
                        minY = point.y;
                    if (point.z < minZ)
                        minZ = point.z;
                    if (point.x > maxX)
                        maxX = point.x;
                    if (point.y > maxY)
                        maxY = point.y;
                    if (point.z > maxZ)
                        maxZ = point.z;
                }
            }
            catch (e_1_1) { e_1 = { error: e_1_1 }; }
            finally {
                try {
                    if (angles_1_1 && !angles_1_1.done && (_a = angles_1.return)) _a.call(angles_1);
                }
                finally { if (e_1) throw e_1.error; }
            }
            return new AcGeBox3d({ x: minX, y: minY, z: minZ }, { x: maxX, y: maxY, z: maxZ });
        }
        else {
            var numPoints = 100;
            var minX = Infinity, minY = Infinity, minZ = Infinity;
            var maxX = -Infinity, maxY = -Infinity, maxZ = -Infinity;
            for (var i = 0; i <= numPoints; i++) {
                var angle = this.startAngle + this.deltaAngle * (i / numPoints);
                var point = this.getPointAtAngle(angle);
                minX = Math.min(minX, point.x);
                minY = Math.min(minY, point.y);
                minZ = Math.min(minZ, point.z);
                maxX = Math.max(maxX, point.x);
                maxY = Math.max(maxY, point.y);
                maxZ = Math.max(maxZ, point.z);
            }
            // Return the bounding box defined by min and max coordinates
            return new AcGeBox3d({ x: minX, y: minY, z: minZ }, { x: maxX, y: maxY, z: maxZ });
        }
    };
    Object.defineProperty(AcGeEllipseArc3d.prototype, "closed", {
        /**
         * Return true if its start point is identical to its end point. Otherwise, return false.
         */
        get: function () {
            return this.deltaAngle == 0;
        },
        enumerable: false,
        configurable: true
    });
    AcGeEllipseArc3d.prototype.getPoints = function (numPoints) {
        if (numPoints === void 0) { numPoints = 100; }
        var points = [];
        var deltaAngle = this.deltaAngle;
        var startAngle = this.startAngle;
        if (this.closed) {
            deltaAngle = TAU;
            startAngle = 0;
        }
        for (var i = 0; i <= numPoints; i++) {
            var angle = startAngle + deltaAngle * (i / numPoints);
            var point = this.getPointAtAngle(angle);
            points.push(point);
        }
        return points;
    };
    /**
     * Sample this ellipse to a polyline whose chord height is bounded by `options`.
     *
     * Circular ellipses use the closed-form circular segment count. Non-circular
     * ellipses use coarse sampling plus local refinement with an evaluation budget.
     *
     * @param options - Chord-height tessellation options
     */
    AcGeEllipseArc3d.prototype.tessellate = function (options) {
        var _this = this;
        var _a, _b;
        var resolved = AcGeCircArc2d.resolveTessellateOptions(options);
        var sweep = this.closed ? TAU : this.deltaAngle;
        var radius = Math.max(this.majorAxisRadius, this.minorAxisRadius);
        var isFull = Math.abs(sweep) >= TAU - 1e-8;
        var minSegments = (_a = resolved.minSegments) !== null && _a !== void 0 ? _a : (isFull ? 8 : 3);
        if (this.isCircular) {
            var numPoints = AcGeCircArc3d.segmentCount(radius, sweep, __assign(__assign({}, options), { minSegments: minSegments }));
            return this.getPoints(numPoints);
        }
        var deviation = (_b = resolved.deviation) !== null && _b !== void 0 ? _b : AcGeCircArc2d.chordDeviationFromRadius(radius, resolved.circleSides);
        var startAngle = this.closed ? 0 : this.startAngle;
        return AcGeCurve3d.tessellateParametricCurve(startAngle, startAngle + sweep, function (angle) { return _this.getPointAtAngle(angle); }, {
            deviation: Math.max(FLOAT_TOL, deviation),
            minSegments: minSegments,
            maxSegments: resolved.maxSegments
        });
    };
    /**
     * Calculate a point on the ellipse at a given angle.
     * @param angle Input the angle in radians where the point is to be calculated.
     * @returns Return the 3d coordinates of the point on the ellipse.
     */
    AcGeEllipseArc3d.prototype.getPointAtAngle = function (angle) {
        var cosTheta = Math.cos(angle);
        var sinTheta = Math.sin(angle);
        var tmp = this.minorAxis
            .clone()
            .multiplyScalar(this.minorAxisRadius)
            .multiplyScalar(sinTheta);
        var point = this.majorAxis
            .clone()
            .multiplyScalar(cosTheta * this.majorAxisRadius)
            .add(tmp);
        return new AcGePoint3d(this.center.x + point.x, this.center.y + point.y, this.center.z + point.z);
    };
    /**
     * Returns the nearest point on this ellipse arc to the given point.
     *
     * Uses uniform sampling in parameter space, which is sufficient for object snap.
     *
     * @param point - Query point in WCS.
     * @param samples - Number of interior samples along the arc span.
     */
    AcGeEllipseArc3d.prototype.nearestPoint = function (point, samples) {
        if (samples === void 0) { samples = 72; }
        var query = new AcGePoint3d(point.x, point.y, point.z || 0);
        var best = this.getPointAtAngle(this.startAngle);
        var bestDistSq = query.distanceToSquared(best);
        for (var i = 1; i <= samples; i++) {
            var angle = this.startAngle + (this.deltaAngle * i) / samples;
            var candidate = this.getPointAtAngle(angle);
            var distSq = query.distanceToSquared(candidate);
            if (distSq < bestDistSq) {
                bestDistSq = distSq;
                best = candidate;
            }
        }
        var dStart = query.distanceToSquared(this.startPoint);
        var dEnd = query.distanceToSquared(this.endPoint);
        if (dStart < bestDistSq && dStart <= dEnd)
            return this.startPoint.clone();
        if (dEnd < bestDistSq && dEnd < dStart)
            return this.endPoint.clone();
        return best.clone();
    };
    /**
     * Returns tangent snap point(s) from the given point to this ellipse arc.
     */
    AcGeEllipseArc3d.prototype.tangentPoints = function (point) {
        var _this = this;
        var localP = this._projectPointToLocalPlane(point);
        var angles = this._findSnapParameterAngles(function (qx, qy, cos, sin) {
            var tx = -_this.majorAxisRadius * sin;
            var ty = _this.minorAxisRadius * cos;
            return (qx - localP.x) * ty - (qy - localP.y) * tx;
        });
        return angles.map(function (angle) { return _this.getPointAtAngle(angle); });
    };
    /**
     * Returns perpendicular snap point(s) on this ellipse arc from the given point.
     */
    AcGeEllipseArc3d.prototype.perpendicularPoints = function (point) {
        var _this = this;
        var localP = this._projectPointToLocalPlane(point);
        var angles = this._findSnapParameterAngles(function (qx, qy, cos, sin) {
            var nx = _this.minorAxisRadius * cos;
            var ny = _this.majorAxisRadius * sin;
            return (qx - localP.x) * ny - (qy - localP.y) * nx;
        });
        return angles.map(function (angle) { return _this.getPointAtAngle(angle); });
    };
    AcGeEllipseArc3d.prototype._projectPointToLocalPlane = function (point) {
        var query = new AcGePoint3d(point.x, point.y, point.z || 0);
        var offset = new AcGeVector3d(query).sub(this.center);
        var distToPlane = offset.dot(this.normal);
        offset.sub(this.normal.clone().multiplyScalar(distToPlane));
        return {
            x: offset.dot(this.majorAxis),
            y: offset.dot(this.minorAxis)
        };
    };
    AcGeEllipseArc3d.prototype._findSnapParameterAngles = function (errorAt, samples) {
        if (samples === void 0) { samples = 144; }
        var a = this.majorAxisRadius;
        var b = this.minorAxisRadius;
        var start = this.startAngle;
        var delta = this.deltaAngle;
        var angles = [];
        var evalError = function (theta) {
            var cos = Math.cos(theta);
            var sin = Math.sin(theta);
            return errorAt(a * cos, b * sin, cos, sin);
        };
        var prevT = start;
        var prevE = evalError(prevT);
        for (var i = 1; i <= samples; i++) {
            var t = start + (delta * i) / samples;
            var e = evalError(t);
            if (Math.abs(prevE) < 1e-10)
                angles.push(prevT);
            if (prevE * e < 0) {
                var lo = prevT;
                var hi = t;
                for (var j = 0; j < 32; j++) {
                    var mid = (lo + hi) / 2;
                    if (evalError(lo) * evalError(mid) <= 0)
                        hi = mid;
                    else
                        lo = mid;
                }
                angles.push((lo + hi) / 2);
            }
            prevT = t;
            prevE = e;
        }
        return angles;
    };
    /**
     * Determines whether a given point is inside the ellipse.
     * @param point - The 3D point to check.
     * @returns - True if the point is inside the ellipse, false otherwise.
     */
    AcGeEllipseArc3d.prototype.contains = function (point) {
        var toPoint = new AcGeVector3d(point).sub(this.center);
        var majorDist = toPoint.dot(this.majorAxis);
        var minorDist = toPoint.dot(this.minorAxis);
        var normalizedMajorDist = majorDist / this.majorAxisRadius;
        var normalizedMinorDist = minorDist / this.minorAxisRadius;
        return (normalizedMajorDist * normalizedMajorDist +
            normalizedMinorDist * normalizedMinorDist <=
            1);
    };
    /**
     * @inheritdoc
     */
    AcGeEllipseArc3d.prototype.transform = function (_matrix) {
        var matrix = _matrix;
        var transformedCenter = this.center.clone().applyMatrix4(matrix);
        var transformedMajorPoint = this.getPointAtAngle(0)
            .clone()
            .applyMatrix4(matrix);
        var transformedMinorPoint = this.getPointAtAngle(Math.PI / 2)
            .clone()
            .applyMatrix4(matrix);
        var majorVector = new AcGeVector3d(transformedMajorPoint).sub(transformedCenter);
        var transformedMinorVector = new AcGeVector3d(transformedMinorPoint).sub(transformedCenter);
        var majorAxisRadius = majorVector.length();
        var minorAxisRadius = transformedMinorVector.length();
        var majorAxis = majorVector.clone().normalize();
        var normal = new AcGeVector3d()
            .crossVectors(majorVector, transformedMinorVector)
            .normalize();
        var minorAxis = new AcGeVector3d()
            .crossVectors(normal, majorAxis)
            .normalize();
        if (minorAxis.dot(transformedMinorVector) < 0) {
            normal.negate();
            minorAxis = new AcGeVector3d().crossVectors(normal, majorAxis).normalize();
        }
        var getAngle = function (point) {
            var relative = new AcGeVector3d(point).sub(transformedCenter);
            var x = relative.dot(majorAxis);
            var y = relative.dot(minorAxis);
            return AcGeMathUtil.normalizeAngle(Math.atan2(y / minorAxisRadius, x / majorAxisRadius));
        };
        var transformedEllipse = this.closed
            ? new AcGeEllipseArc3d(transformedCenter, normal, majorAxis, majorAxisRadius, minorAxisRadius, 0, TAU)
            : new AcGeEllipseArc3d(transformedCenter, normal, majorAxis, majorAxisRadius, minorAxisRadius, getAngle(this.startPoint.clone().applyMatrix4(matrix)), getAngle(this.endPoint.clone().applyMatrix4(matrix)));
        this.center = transformedEllipse.center;
        this.normal = transformedEllipse.normal;
        this.majorAxis = transformedEllipse.majorAxis;
        this.majorAxisRadius = transformedEllipse.majorAxisRadius;
        this.minorAxisRadius = transformedEllipse.minorAxisRadius;
        this._startAngle = transformedEllipse._startAngle;
        this._endAngle = transformedEllipse._endAngle;
        this._boundingBoxNeedsUpdate = true;
        return this;
    };
    /**
     * @inheritdoc
     */
    AcGeEllipseArc3d.prototype.copy = function (value) {
        this.center = value.center;
        this.normal = value.normal;
        this.majorAxis = value.majorAxis;
        this.majorAxisRadius = value.majorAxisRadius;
        this.minorAxisRadius = value.minorAxisRadius;
        this.startAngle = value.startAngle;
        this.endAngle = value.endAngle;
        this._boundingBoxNeedsUpdate = true;
        return this;
    };
    /**
     * @inheritdoc
     */
    AcGeEllipseArc3d.prototype.clone = function () {
        return new AcGeEllipseArc3d(this.center, this.normal, this.majorAxis, this.majorAxisRadius, this.minorAxisRadius, this.startAngle, this.endAngle);
    };
    Object.defineProperty(AcGeEllipseArc3d.prototype, "plane", {
        /**
         * Return the plane in which the ellipse arc lies.
         */
        get: function () {
            var distance = new AcGeVector3d(this.center).distanceTo(ORIGIN_POINT_3D);
            return new AcGePlane(this.normal, distance);
        },
        enumerable: false,
        configurable: true
    });
    /**
     * Returns a copy with major and minor radii adjusted by `offsetDist`.
     *
     * @param offsetDist - Signed offset distance in drawing units
     * @returns Offset ellipse arc, or `null` when either radius is not positive
     */
    AcGeEllipseArc3d.prototype.offset = function (offsetDist) {
        var majorAxisRadius = this.majorAxisRadius + offsetDist;
        var minorAxisRadius = this.minorAxisRadius + offsetDist;
        if (majorAxisRadius <= 0 || minorAxisRadius <= 0)
            return null;
        return new AcGeEllipseArc3d(this.center, this.normal, this.majorAxis, majorAxisRadius, minorAxisRadius, this.startAngle, this.endAngle);
    };
    return AcGeEllipseArc3d;
}(AcGeCurve3d));
export { AcGeEllipseArc3d };
//# sourceMappingURL=AcGeEllipseArc3d.js.map