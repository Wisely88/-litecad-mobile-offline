 ...
 */
function acgeClamp(value, min, max) {
 ...
 */
function acgeEuclideanModulo(n, m) {
    return ((n % m) + m) % m;
}
/**
 ...
 */
function acgeMapLinear(x, a1, a2, b1, b2) {
 ...
 */
function acgeInverseLerp(x, y, value) {
    if (x !== y) {
        return (value - x) / (y - x);
    }
    else {
 ...
 */
function acgeDamp(x, y, lambda, dt) {
 ...
 */
function acgePingpong(x, length) {
 ...
 */
function acgeSmoothstep(x, min, max) {
    if (x <= min)
        return 0;
    if (x >= max)
        return 1;
 ...
 * @returns Return a value between 0-1
 */
function acgeSmootherstep(x, min, max) {
    if (x <= min)
        return 0;
    if (x >= max)
        return 1;
...
function acgeRandFloat(low, high) {
 ...
 */
function acgeRandFloatSpread(range) {
 ...
 */
function acgeIsPowerOfTwo(value) {
 ...
 */
function acgeCeilPowerOfTwo(value) {
 ...
 */
function acgeFloorPowerOfTwo(value) {
 ...
 */
function acgeNormalizeAngle(angle) {
    var TAU = Math.PI * 2;
 ...
 */
function acgeIsBetween(valueToTest, value1, value2) {
 ...
 */
function acgeIsBetweenAngle(angleToTest, startAngle, endAngle, clockwise) {
 ...
 */
function acgeIsAngleOnCcwSweep(start, mid, end) {
    var total = acgeNormalizeAngle(end - start);
    var offset = acgeNormalizeAngle(mid - start);
    return offset <= total + 1e-7;
}
function acgeIntPartLength(num) {
    num = Math.abs(num);
    if (num < 1.0) {
        return 0;
    }
...
function acgeRelativeEps(num, epsilon) {
    if (epsilon === void 0) { epsilon = 1.0e-7; }
    var count = acgeIntPartLength(num);
...
export { DEG2RAD, RAD2DEG, acgeGenerateUUID, acgeClamp, acgeEuclideanModulo, acgeMapLinear, acgeInverseLerp, acgeLerp, acgeDamp, acgePingpong, acgeSmoothstep, acgeSmootherstep, acgeRandInt, acgeRandFloat, acgeRandFloatSpread, acgeSeededRandom, acgeDegToRad, acgeRadToDeg, acgeIsPowerOfTwo, acgeCeilPowerOfTwo, acgeFloorPowerOfTwo, acgeNormalizeAngle, acgeIsBetween, acgeIsBetweenAngle, acgeIsAngleOnCcwSweep, acgeIntPartLength, acgeRelativeEps, AcGeMathUtil };
