 ...
 */
export function acgeGenerateUniformKnots(degree, numControlPoints) {
    var knots = [];
 ...
 */
export function acgeGenerateChordKnots(degree, points) {
 ...
 */
export function acgeComputeParameterValues(points, parameterization) {
 ...
 */
export function acgeGenerateAveragedKnots(degree, parameters) {
 ...
 */
export function acgeGenerateSqrtChordKnots(degree, points) {
    var n = points.length - 1;
    var p = degree;
 ...
 */
export function acgeInterpolateNurbsCurve(fitPoints, degree, parameterization, startTangent, endTangent) {
    ...
    var params = acgeComputeParameterValues(safePoints, parameterization);
    ...
    var knots = acgeGenerateAveragedKnots(degree, extendedParams);
    var size = n + 1;
 ...
 */
export function acgeBasisFunction(i, k, u, knots) {
    if (k === 0) {
    ...
    return (c1 * acgeBasisFunction(i, k - 1, u, knots) +
        c2 * acgeBasisFunction(i + 1, k - 1, u, knots));
}
/**
 * Calculate point on NURBS curve
 */
export function acgeEvaluateNurbsPoint(u, degree, knots, controlPoints, weights) {
    ...
    var weight = 0;
    for (var i = 0; i <= n; i++) {
        var basis = acgeBasisFunction(i, p, u, knots);
 ...
 */
function basisFunctionDeriv1(i, k, u, knots) {
    if (k === 0)
        return 0;
    var denomA = knots[i + k] - knots[i];
    var denomB = knots[i + k + 1] - knots[i + 1];
    var value = 0;
    if (denomA > 1e-10) {
 ...
 */
function basisFunctionDeriv2(i, k, u, knots) {
    if (k <= 1)
        return 0;
    var denomA = knots[i + k] - knots[i];
    var denomB = knots[i + k + 1] - knots[i + 1];
 ...
 */
export function acgeEvaluateNurbsDerivatives(u, degree, knots, controlPoints, weights) {
    var n = controlPoints.length - 1;
    var p = degree;
    var startParam = knots[p];
    var endParam = knots[n + 1];
    ...
    for (var i = 0; i <= n; i++) {
        var basis = acgeBasisFunction(i, p, u, knots);
        var dbasis = basisFunctionDeriv1(i, p, u, knots);
        var d2basis = basisFunctionDeriv2(i, p, u, knots);
        var weight = weights[i];
        var wBasis = weight * basis;
        var wDBasis = weight * dbasis;
        var wD2Basis = weight * d2basis;
    ...
    if (Math.abs(w) < 1e-10) {
        var point_1 = acgeEvaluateNurbsPoint(u, degree, knots, controlPoints, weights);
        return { point: point_1, deriv1: [0, 0, 0], deriv2: [0, 0, 0] };
    }
    var w2 = w * w;
    var point = [aw[0] / w, aw[1] / w, aw[2] / w];
    var deriv1 = [
        (daw[0] * w - aw[0] * dw) / w2,
        (daw[1] * w - aw[1] * dw) / w2,
        (daw[2] * w - aw[2] * dw) / w2
    ];
    var num2x = d2aw[0] * w - aw[0] * d2w;
    var num2y = d2aw[1] * w - aw[1] * d2w;
    var num2z = d2aw[2] * w - aw[2] * d2w;
    var deriv2 = [
        num2x / w2 - (2 * dw * deriv1[0]) / w,
        num2y / w2 - (2 * dw * deriv1[1]) / w,
        num2z / w2 - (2 * dw * deriv1[2]) / w
    ];
    return { point: point, deriv1: deriv1, deriv2: deriv2 };
}
/**
 * Signed planar curvature from parametric derivatives in XY.
 */
export function acgeSignedPlanarCurvature(deriv1, deriv2) {
    var dx = deriv1[0];
    var dy = deriv1[1];
    var ddx = deriv2[0];
    var ddy = deriv2[1];
 ...
 */
export function acgeCalculateCurveLength(degree, knots, controlPoints, weights) {
    var p = degree;
    var startParam = knots[p];
    var endParam = knots[knots.length - p - 1];
    var length = 0;
    ...
    var prevPoint = acgeEvaluateNurbsPoint(startParam, degree, knots, controlPoints, weights);
    for (var i = 1; i <= steps; i++) {
        var u = startParam + i * step;
        var point = acgeEvaluateNurbsPoint(u, degree, knots, controlPoints, weights);
    ...
    var finalPoint = acgeEvaluateNurbsPoint(endParam, degree, knots, controlPoints, weights);
 ...
 */
export function acgeInterpolateControlPoints(fitPoints, degree, parameterization, startTangent, endTangent) {
    if (degree === void 0) { degree = 3; }
    if (parameterization === void 0) { parameterization = 'Uniform'; }
    if (fitPoints.length === 0) {
        return [];
    }
    return acgeInterpolateNurbsCurve(fitPoints, degree, parameterization, startTangent, endTangent).controlPoints;
}
