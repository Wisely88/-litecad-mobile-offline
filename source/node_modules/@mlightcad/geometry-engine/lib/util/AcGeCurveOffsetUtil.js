...
import { acgeOffsetPolyline2d, acgePreparePolylineForOffset } from '../geometry/AcGePolyline2dOffset';
 ...
 */
export function acgeOffsetPointByDirectionInXY(point, direction, offsetDist) {
 ...
 */
export function acgeOffsetVertexPath(points, closed, offsetDist) {
    var _a;
    if (points.length < 2)
        return null;
    var polyline = acgePreparePolylineForOffset(new AcGePolyline2d(points.map(function (point) { return ({ x: point.x, y: point.y }); }), closed));
    var results = acgeOffsetPolyline2d(polyline, offsetDist);
